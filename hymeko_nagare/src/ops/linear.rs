...
impl LinearLayer {
    ...
    pub fn new(in_dim: usize, out_dim: usize, seed: u64) -> Self {
        ...
        for v in &mut w { *v = (rng.random::<f32>() * 2.0 - 1.0) * scale; }
        Self {
            w,
            b: vec![0.0; out_dim],
            in_dim,
...
pub fn linear_forward(layer: &LinearLayer, x: &[f32]) -> Vec<f32> {
    ...
    out.par_chunks_mut(layer.out_dim).enumerate().for_each(|(ni, row)| {
...
pub fn linear_backward(
    layer: &LinearLayer, x: &[f32], grad_y: &[f32],
) -> (Vec<f32>, LinearLayer) {
    let n = x.len() / layer.in_dim;
    ...
    grad_x.par_chunks_mut(layer.in_dim).enumerate().for_each(|(ni, gx_row)| {
    ...
    fn linear_backward_matches_numerical() {
        ...
        for i in 0..layer.in_dim {
            for j in 0..layer.out_dim {
                ...
                let mut l_p = layer.clone(); l_p.w[idx] += eps;
                let mut l_m = layer.clone(); l_m.w[idx] -= eps;
                ...
                assert!((ana - num).abs() < 1e-2,
                    "w[{},{}]: ana={} num={}", i, j, ana, num);
            }
        }
        // Check grad_b
        for j in 0..layer.out_dim {
            let mut l_p = layer.clone(); l_p.b[j] += eps;
            let mut l_m = layer.clone(); l_m.b[j] -= eps;
            let y_p: f32 = linear_forward(&l_p, &x).iter().sum();
            let y_m: f32 = linear_forward(&l_m, &x).iter().sum();
            let num = (y_p - y_m) / (2.0 * eps);
        ...
        // Check grad_x
        for i in 0..x.len() {
            let mut x_p = x.clone(); x_p[i] += eps;
            let mut x_m = x.clone(); x_m[i] -= eps;
