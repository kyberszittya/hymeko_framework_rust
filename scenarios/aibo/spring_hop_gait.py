...
Each stride cycle:
  ...
  2. **LAUNCH**  — release (actuator off → the springs fire = a passive vertical lift) while a
     **motor-limited** hip-flex push (≤ ``hip_tau_cap`` N·m) drives the body forward; the push
     magnitude is scaled by the remaining distance, so it eases off into the goal.
  3. **CATCH**   — a *short*, momentum-preserving catch: PD-hold only the **upright-critical joints**
     (knees for height, hip-abduction for roll) back to q0, while the hip-flex joints carry a small
     **sustained forward drive** (motor-limited, easing off near the goal). The body does **not**
     brake to a stand — the forward momentum passes through into the next stride, so the gait flows
     forward continuously instead of stopping to re-stabilize between hops.

Honesty. The **vertical lift is the passive spring** (fast knee, not a commanded motor — legitimate
series elasticity, unlike the retracted 27 rad/s exploit). The **horizontal drive is a motor-limited
hip torque** (≤ a realistic ~5 N·m push, ≤ 8 N·m for the catch), not an injected velocity. The body
stays upright throughout and keeps its forward momentum between strides.
"""
...
class SpringHopGait:
    ...
    catch_drive: float = 3.5     # sustained forward hip drive during the catch — carries momentum
    motor_tau_cap: float = 8.0   # realistic AIBO-class leg motor — ALL actuator torque is clamped here
    max_hops: int = 20
    launch_steps: int = 95
    push_steps: int = 70
    catch_steps: int = 70        # SHORT catch — no braking to a stand
    arrive_settle: int = 220     # once reached, come to an upright rest AT the goal
    ...
    def _hold_posture(self, d: mujoco.MjData, q0: dict[str, float],
    ...
    def run(self, on_step: StepHook | None = None) -> HopToGoalResult:
        ...
        hip_flex = frozenset(f"hip_flex_{leg}" for leg in LEGS)
        ...
        while (goal_x - float(d.xpos[torso, 0])) > self.reach_radius and hops < self.max_hops:
            ...
            ease = float(np.tanh(6.0 * remaining))              # 1 far from goal -> 0 approaching it
            push = float(np.clip(6.0 * remaining, -self.hip_tau_cap, self.hip_tau_cap))
            drive = float(np.clip(self.catch_drive * ease, -self.motor_tau_cap, self.motor_tau_cap))
            ...
            for _ in range(self.catch_steps):                   # CATCH: keep upright, KEEP momentum
                self._hold_posture(d, q0, exclude=hip_flex)     # knees + abduction only (height + roll)
                for leg in LEGS:                                # hip-flex: small sustained forward drive
                    d.qfrc_applied[self._vadr[f"hip_flex_{leg}"]] = drive
                mujoco.mj_step(m, d)
                min_up = min(min_up, self._upright(d, torso))
                if on_step:
                    on_step(d, "catch", hops)
            hop_x.append(float(d.xpos[torso, 0]) - x_start)
        reached = (goal_x - float(d.xpos[torso, 0])) <= self.reach_radius
        if reached:                                             # ARRIVE: come to an upright rest AT the goal
            for _ in range(self.arrive_settle):
                self._hold_posture(d, q0)
                mujoco.mj_step(m, d)
                min_up = min(min_up, self._upright(d, torso))
                if on_step:
                    on_step(d, "arrive", hops)
