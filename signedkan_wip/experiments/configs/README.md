...
```
python -m signedkan_wip.experiments.run --config <name>.yaml
```

The framework (under `signedkan_wip/src/experiments/`) replaces
~221 historical launcher scripts under `signedkan_wip/experiments/run_*.{sh,py}`.
...
1. **Discovery first** (CLAUDE.md §6.5 #12): grep this dir + the old
   `signedkan_wip/experiments/run_*.{sh,py}` for the dataset/concept;
...
`signedkan_wip/src/experiments/config.py` is the source of truth.

## Migration backlog (out of scope this commit)

~78 shell launchers + ~108 Python launchers under
`signedkan_wip/experiments/` are NOT yet migrated. They continue to
