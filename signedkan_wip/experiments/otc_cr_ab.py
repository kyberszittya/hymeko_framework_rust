...
Answers "OTC should use CR": trains the *same* pairwise signed-KAN architecture (``signed_kan.SignedGraphHSiKAN``)
with ``activation="cr"`` vs ``"bspline"`` on the same data + split, and reports AUC / macro-F1. Uses the legacy
loader+split (``signedkan_wip.src.datasets.legacy``) so the split is identical to the published runs; the message-
passing adjacency is built from **train edges only** (no test-label leakage). CPU-friendly, sparse backend.

    python -m signedkan_wip.experiments.otc_cr_ab --dataset bitcoin_otc --activation cr --epochs 200
    python -m signedkan_wip.experiments.otc_cr_ab --smoke          # both splines, few epochs, quick check
"""
