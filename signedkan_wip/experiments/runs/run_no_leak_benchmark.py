...
# Subprocess strict runners (CLI + single-line JSON, --shuffle-train-signs):
# the structural prior via run_gomb_smoke (train-only cycle pool, audit-clean).
SUBPROC_RUNNERS = {
    "gomb": ("signedkan_wip.experiments.runs.run_gomb_smoke", "val_auroc"),
}
...
def _run_subprocess(cell: Cell, seed: int, shuffle: bool) -> tuple[float | None, int | None]:
    ...
    module, auc_key = SUBPROC_RUNNERS[cell.model]
    # Pinned narrow-deep arch: width -> --d-middle, depth -> --middle-n-layers
    # (StackedMiddleHSiKAN dispatches when depth >= 2).
    cmd = [sys.executable, "-m", module, "--dataset", cell.dataset,
           "--seed", str(seed), "--n-epochs", str(cell.n_epochs),
           "--d-middle", str(cell.width), "--middle-n-layers", str(cell.depth)]
    if shuffle:
        cmd.append("--shuffle-train-signs")
    proc = subprocess.run(cmd, capture_output=True, text=True)
    auc, n_params = _parse_result(proc.stdout, auc_key)
    if auc is None:
...
def _result_key(row: dict[str, Any]) -> tuple[str, str, int, bool]:
    """Resume identity of a result row: (dataset, model, seed, shuffle).

    Tolerates legacy rows written before the ``seed`` field existed (treated as
    seed 0), so an old single-seed JSONL still resumes cleanly.
    """
...
def main(
    ...
    out_path = Path(out_path) if out_path else Path(
        f"signedkan_wip/experiments/results/no_leak_{'smoke' if smoke else 'e1'}.jsonl")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    rows, done = _load_done(out_path)
    print(f"No-leakage {'smoke' if smoke else 'E1'}: {len(cells)} cells × "
...
if __name__ == "__main__":
    ...
    if a.seeds:
        ...
        seeds = tuple(range(5)) if a.full else (0,)
