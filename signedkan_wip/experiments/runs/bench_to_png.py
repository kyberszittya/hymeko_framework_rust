...
Usage:
    python -m signedkan_wip.experiments.runs.bench_to_png
    python -m signedkan_wip.experiments.runs.bench_to_png --device cpu --out fig.png
"""
...
def main(json_path: Any = None, device: str = "cpu", out_path: Any = None) -> Path:
    ...
    out_path = Path(out_path) if out_path else Path(
        f"signedkan_wip/experiments/results/inference_bench_{device}.png")
...
def main_all(json_path: Any = None, results_dir: Any = None) -> list[Path]:
    ...
    from signedkan_wip.experiments.runs.sota_compare import (
