...
table is produced by :mod:`signedkan_wip.experiments.eval.aggregate_protocol_match`.

One file, mode arg (§6.5 #13):
    python -m signedkan_wip.experiments.runs.run_protocol_matched_sigat --smoke
    python -m signedkan_wip.experiments.runs.run_protocol_matched_sigat --full
"""
...
from signedkan_wip.experiments.runs.run_baseline_audit import run_audit
from signedkan_wip.experiments.runs.run_hsikan_rotor import TrainConfig, run as run_hsikan
from signedkan_wip.experiments.runs.run_no_leak_benchmark import (
    RSS_CAP_BYTES,
    audit_gate,
)
...
def main(smoke: bool = True, out_path: Any = None,
    ...
    out_path = Path(out_path) if out_path else Path(
        f"signedkan_wip/experiments/results/protocol_matched_{label}.jsonl")
