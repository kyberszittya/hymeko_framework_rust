"""CLI entry: ``python -m signedkan_wip.experiments.run --config <yaml>``
...
def main(argv: list[str] | None = None) -> int:
    ...
    from signedkan_wip.src.experiments import ExperimentConfig
    from signedkan_wip.src.experiments.registry import ExperimentRegistry
    from signedkan_wip.src.experiments.runner import run_from_config

    # Trigger subclass registration (the runner module's import
    # populates the registry via __init_subclass__).
    import signedkan_wip.src.experiments.runner  # noqa: F401
