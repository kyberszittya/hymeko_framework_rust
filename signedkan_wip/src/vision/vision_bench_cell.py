...
Usage
-----
    python -m signedkan_wip.src.vision.vision_bench_cell \\
...
def build_model(name: str, *, h: int, w: int, n_classes: int, hidden: int,
    ...
    """
    if name == "cnn":
        from signedkan_wip.src.vision.neocog_hgnn import TinyCNN

        return TinyCNN(n_classes, hidden=hidden)
    if name == "mlp":
        from signedkan_wip.src.vision.neocog_hgnn import MLP

        return MLP(h * w, n_classes, hidden=hidden)
    if name == "hgnn":
        from signedkan_wip.src.vision.neocog_hgnn import NeocogHGNN

        return NeocogHGNN(h, w, n_classes, hidden=hidden)
    if name == "hsikan":
        from signedkan_wip.src.vision.hsikan_vision import HSiKANVisionClassifier
    ...
    if name == "fuzzy_sig":
        from signedkan_wip.src.vision.fuzzy_signature import (
