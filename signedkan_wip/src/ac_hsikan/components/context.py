...
* CliffordFIRContext  -- causal multivector FIR filter from the Gömb
                          architecture (signedkan_wip/src/sequence/
...
class CliffordFIRContext(ContextEncoder):
    ...
    ``signedkan_wip.src.triton_kernels.clifford_fir`` (4-5× forward,
    ...
    def forward(self, x):
        ...
        if x.is_cuda:
            from signedkan_wip.src.triton_kernels.clifford_fir import (
                clifford_fir_triton,
            )
