...
``signedkan_wip/paper/figures/``.

Run::

  uv run python -m signedkan_wip.src.paperkit.build_leakage_source_figure
  uv run python -m signedkan_wip.src.paperkit.build_leakage_source_figure --self-check
"""
...
RESULTS = REPO / "signedkan_wip" / "experiments" / "results"
