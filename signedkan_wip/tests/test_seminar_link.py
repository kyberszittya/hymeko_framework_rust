...
the default, non-``ml`` environment). Run with::
    ...
    uv run --group ml --group dev pytest -p no:randomly \
        signedkan_wip/tests/test_seminar_link.py
"""
from __future__ import annotations

import pytest

from signedkan_wip.demos.seminar.base import REPO_ROOT, DemoRunner
from signedkan_wip.demos.seminar.cli import build_parser
from signedkan_wip.demos.seminar.compat import register_legacy_checkpoint_aliases
...
def test_legacy_alias_is_idempotent() -> None:
    ...
    assert "signedkan_wip.src.signedkan" in sys.modules
    ...
    assert first == [] or "signedkan_wip.src.signedkan" in first
