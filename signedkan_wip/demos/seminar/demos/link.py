...
if TYPE_CHECKING:  # avoid importing torch-backed modules at load time.
    from signedkan_wip.src.demo.inference import ModelBundle, PredictionResult
...
class LinkDemo:
    ...
    def _resolve_checkpoint(self, args: argparse.Namespace) -> tuple[Path, float | None]:
        ...
        from signedkan_wip.src.demo.registry import load_registry, ModelEntry
    ...
    def run(self, args: argparse.Namespace, ctx: DemoContext) -> DemoResult:
        ...
        from signedkan_wip.src.demo.inference import load_bundle, predict_test_edges
    ...
    def _render(bundle: "ModelBundle", pred: "PredictionResult", out_dir: Path) -> list[Path]:
        from signedkan_wip.src.demo.plotting import alpha_figure, roc_figure
