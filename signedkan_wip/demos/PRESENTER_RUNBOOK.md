...
```powershell
PYTHONPATH=. .venv/Scripts/python.exe -m signedkan_wip.demos.seminar link `
...
```powershell
PYTHONPATH=. .venv/Scripts/python.exe -m signedkan_wip.demos.seminar hive `
...
```powershell
PYTHONPATH=. .venv/Scripts/python.exe -m signedkan_wip.demos.seminar latency `
    --device cpu --seed 0
```
...
- SGCN vs HSiKAN-lean (h=4) vs HSiKAN-joint (h=16) bars; ≥5 repeats (20 timed +
  ...
  -m signedkan_wip.experiments.runs.run_inference_bench` then `… .bench_to_png`.
...
```powershell
PYTHONPATH=. .venv/Scripts/python.exe -m signedkan_wip.demos.seminar balance `
