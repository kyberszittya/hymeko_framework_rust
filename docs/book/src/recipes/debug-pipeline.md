...
`hymeko_neuro/eval/test_harness/` ships a 3-layer integrity-test framework that catches the 5 bug classes that have cost overnight GPU time:
...
```python
# hymeko_neuro/eval/test_harness/env_var_audit.py — extend cases()
...
- `hymeko_neuro/eval/test_harness/` — three modules: `env_var_audit`, `result_capture`, `invariant_check`
