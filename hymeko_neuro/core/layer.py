...
:class:`~hymeko_neuro.core.backbone.SignedKANBackbone`). This is the shared core that ``hymeko_rl`` (dense/inductive) and,
after phase 3, ``hymeko_neuro`` (sparse/transductive) both build on.
"""
...
class SignedKANLayer(nn.Module):
    ...
    The signed aggregation is delegated to an :class:`~hymeko_neuro.core.backends.AggregationBackend` so the same layer
