...
Usage:
    python -m hymeko_neuro.paperkit.analyze_cprofile /path/to/cprofile.prof
"""
...
def by_file(stats_obj: pstats.Stats, min_tot: float = 1.0):
    ...
    'hymeko_neuro' or top-level libraries we care about)."""
    ...
    for func, (cc, nc, tt, ct, callers) in stats_obj.stats.items():
        ...
        if path == "~":
            ...
            for needle in ("hymeko_neuro/", "torch/", "triton/", "numpy/",
                            "pytorch", "rayon", "site-packages/"):
                if needle in path:
                    key = needle.rstrip("/")
                    if needle == "hymeko_neuro/":
                        key = path.split("hymeko_neuro/")[-1]
