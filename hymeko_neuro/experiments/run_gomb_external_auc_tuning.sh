...
if [[ "$RUN_BITCOIN" == "1" ]]; then
  ...
  python -m hymeko_neuro.experiments.runs.run_gomb_tune \
...
python -m hymeko_neuro.experiments.gomb_jsonl_summarize "${OUTDIR}/gomb_tune_external_joint.jsonl" 2>/dev/null || true
python -m hymeko_neuro.experiments.gomb_jsonl_summarize "${OUTDIR}/gomb_tune_external_vanilla.jsonl" 2>/dev/null || true
if [[ "$RUN_SLASHDOT" == "1" ]]; then
  python -m hymeko_neuro.experiments.gomb_jsonl_summarize "${OUTDIR}/gomb_tune_external_slashdot_joint.jsonl" 2>/dev/null || true
  python -m hymeko_neuro.experiments.gomb_jsonl_summarize "${OUTDIR}/gomb_tune_external_slashdot_vanilla.jsonl" 2>/dev/null || true
fi
