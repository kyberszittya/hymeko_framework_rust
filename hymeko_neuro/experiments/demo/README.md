...
```bash
PYTHONPATH=. uv run python -m hymeko_neuro.run_final_cell \
...
```bash
PYTHONPATH=. uv run python -m hymeko_neuro.experiments.demo.gui
```
...
```bash
# localhost only
HYMEKO_DEMO_HOST=127.0.0.1 PYTHONPATH=. uv run python -m hymeko_neuro.experiments.demo.gui

# pick a non-default port
PYTHONPATH=. uv run python -m hymeko_neuro.experiments.demo.gui --port 8080

# public tunnel (Gradio-hosted; URL printed on launch)
PYTHONPATH=. uv run python -m hymeko_neuro.experiments.demo.gui --share
```
...
```bash
PYTHONPATH=. uv run python -m hymeko_neuro.experiments.demo.kinematic_classifier \
...
- **Model is re-imported by string.** `model_class` is a qualified import
  path, e.g. `hymeko_neuro.models.mixed_arity_signedkan.model.MixedAritySignedKAN`.
...
- **Round-trip tests** live at
  `hymeko_neuro/tests/test_demo_checkpoint.py`. They cover save / load
  with and without the bundle + external classifier.

## See also

- `hymeko_neuro/run_final_cell.py` — training entry point.
- `hymeko_neuro/models/mixed_arity_signedkan/` — model definition.
