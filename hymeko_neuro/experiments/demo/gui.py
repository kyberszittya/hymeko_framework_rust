    ...
    with gr.Blocks(title="HyMeKo signed link prediction — demo") as app:
        ...
        if catalogue_available:
            ...
            gr.Markdown(
                ...
                f"Edit `hymeko_neuro/experiments/demo/models.yaml` to add more, or "
                f"set `HYMEKO_MODEL_REGISTRY=...` to use a different file."
            )
        else:
            gr.Markdown(
                "ℹ No model registry found at "
                "`hymeko_neuro/experiments/demo/models.yaml`. Falling back to "
                "upload-only mode."
            )
            ...
            with gr.Tab("Kinematic graph"):
                ...
                with gr.Row():
                    urdf_dd = gr.Dropdown(
                        ...
                        info="Edit `hymeko_neuro/experiments/demo/kinematic_models.yaml` "
