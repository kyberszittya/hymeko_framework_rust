...
#
# Usage:
#   bash hymeko_neuro/experiments/vision/launch_demo.sh             # auto: latest CMNIST ckpt or quick-train
#   bash hymeko_neuro/experiments/vision/launch_demo.sh a2          # train Stage A-2 then launch
#   bash hymeko_neuro/experiments/vision/launch_demo.sh b_resnet    # train Stage B then launch
#   bash hymeko_neuro/experiments/vision/launch_demo.sh b_hsikan    # train Stage B' (HSiKAN-CR) then launch
#   bash hymeko_neuro/experiments/vision/launch_demo.sh c_fpn       # train Stage C then launch
#   bash hymeko_neuro/experiments/vision/launch_demo.sh quick       # force quick-train (no save)
#   bash hymeko_neuro/experiments/vision/launch_demo.sh list        # list known ckpts and exit
#
...
list_ckpts() {
  ...
  echo "  hymeko_neuro/experiments/results/*/ckpts/)"
  echo
  echo "Other on-disk checkpoints (NOT load-able — different schema):"
  find "$REPO_ROOT/hymeko_neuro/experiments/results" \
...
# subdirs under hymeko_neuro/experiments/results/. Excludes VOC ckpts
...
case "$ACTION" in
  ...
  auto)
    ...
    if [ -n "$ckpt" ] && [ -f "$ckpt" ]; then
      ...
      python -m hymeko_neuro.experiments.vision.demo_hymeyolo_tk --checkpoint "$ckpt"
    else
