...
if [[ -z "${OPTUNA_STORAGE:-}" ]]; then
  export OPTUNA_STORAGE="sqlite:///hymeko_neuro/experiments/results/optuna_serial_${STAMP}.db"
fi
...
run_one() {
  ...
  python -m hymeko_neuro.experiments.runs.run_optuna_search \
    --dataset "${ds}" \
    --n-trials "${N_TRIALS}" \
    --n-epochs "${N_EPOCHS}" \
...
if [[ -n "${OPTUNA_QUEUE_REST_AT:-}" ]]; then
  ...
  JOBDIR="${REPO}/hymeko_neuro/experiments/results"
  mkdir -p "${JOBDIR}"
  JOBFILE="${JOBDIR}/.optuna_at_${STAMP}_$$.sh"
  {
    ...
    echo "exec bash $(printf %q "${REPO}/hymeko_neuro/experiments/run_optuna_serial_datasets.sh")"
  } >"${JOBFILE}"
  chmod +x "${JOBFILE}"
