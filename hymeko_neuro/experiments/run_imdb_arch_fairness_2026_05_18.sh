...
#
# Queues behind any in-flight hymeko_neuro training.
...
OUT_DIR="hymeko_neuro/experiments/results/imdb_arch_fairness_${STAMP}"
...
# Queue behind any in-flight hymeko_neuro training.
echo "[orch] waiting for GPU..." | tee -a "$LOG"
while pgrep -af 'hymeko_neuro\.src\.(run_optuna_search|run_final_cell|run_gomb_smoke|vision\.train_circles_ricci|vision\.train_voc|sequence\.(train|run))' \
...
systemd-run --user --scope --quiet \
  ...
  python -u -m hymeko_neuro.experiments.sequence.run_imdb_mlm_pretrain \
...
run_finetune() {
  ...
  local cmd=("python" "-u" "-m" "hymeko_neuro.experiments.sequence.train_imdb_classifier")
  if [ "$arch" = "transformer" ]; then
    cmd=("python" "-u" "-m" "hymeko_neuro.experiments.sequence.train_imdb_transformer")
  fi
  local extra=()
