...
python3 -c "from hymeko_neuro.data.datasets import load; g = load('wikisigned'); print(f'|V|={g.n_nodes} |E|={len(g.edges)}')" 2>&1 | tee -a "$MASTER"
...
TIMEOUT_S=3600 run "wikisigned_baseline_s0" \
    ...
    python -u -m hymeko_neuro.experiments.runs.run_final_cell \
