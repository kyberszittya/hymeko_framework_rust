...
#
# USAGE
#   sbatch hymeko_neuro/experiments/run_adaptive_mv_5seed_2026_05_10.sh
#   (or local: bash hymeko_neuro/experiments/run_adaptive_mv_5seed_2026_05_10.sh)
#
...
if [ -n "${SLURM_JOB_ID:-}" ] && [ -f "$REPO_ROOT/hymeko_signedkan.sif" ]; then
    ...
    if [ -z "${INSIDE_SIF:-}" ]; then
        exec singularity exec --nv \
            ...
            bash -c "cd /workspace && bash hymeko_neuro/experiments/run_adaptive_mv_5seed_2026_05_10.sh"
    fi
fi
