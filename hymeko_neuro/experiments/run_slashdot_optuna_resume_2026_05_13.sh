...
# Waits for the in-flight HyMeYOLO redo (or any hymeko_neuro GPU job) to
...
LOG="hymeko_neuro/experiments/results/optuna_slashdot_resume_20260513T010510Z.log"

echo "=== SLASHDOT RESUME $(date -u +%Y-%m-%dT%H:%M:%SZ) (waiting for HyMeYOLO) ===" >> "$LOG"

# Wait for any hymeko_neuro GPU job to finish.
SELF_PID="$$"
SELF_NAME="$(basename "$0")"
while pgrep -af 'hymeko_neuro\.src\.run_optuna_search|hymeko_neuro\.src\.run_final_cell|hymeko_neuro\.src\.vision\.train_circles_ricci' \
...
bash hymeko_neuro/experiments/run_optuna_serial_datasets.sh >> "$LOG" 2>&1
