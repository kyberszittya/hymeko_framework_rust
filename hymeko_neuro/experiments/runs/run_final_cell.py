...
Usage:
    HSIKAN_TORCH_COMPILE=1 python -m hymeko_neuro.experiments.runs.run_final_cell \
...
from hymeko_neuro.runtime.hsikan_device_env import resolve_hsikan_device
...
def _save_demo_checkpoint(
    ...
    if save_path is None or save_path == "":
        return
    from hymeko_neuro.experiments.demo.checkpoint import (
    ...
    save_checkpoint(
        ...
        model,
        cfg,
        "hymeko_neuro.models.mixed_arity_signedkan.model.MixedAritySignedKAN",
        meta,
...
def cell_signed_graph(dataset: str, model_name: str, hidden: int,
    ...
    from hymeko_neuro.graph.cycle_cache import (
    ...
    from hymeko_neuro.graph.cycle_cache.api import (
        lazy_load_construct_k, lazy_load_construct_walks,
    )
    from hymeko_neuro.graph.cycle_cache.stats import LazyCyclePool
    from hymeko_neuro.data.datasets import load, deduplicate_pairs, split
    from hymeko_neuro.data.datasets import sbm_signed
    from hymeko_neuro.hyperedge.hyperedges import construct  # noqa: F401  (kept for compat; cached_construct_triads is the live path)
    from hymeko_neuro.hyperedge.n_tuples import construct_k  # noqa: F401  (cached_construct_k is the live path)
    from hymeko_neuro.models.mixed_arity_signedkan import (MixedAritySignedKAN,
                                          MixedAritySignedKANConfig,
                                          subsample_tuples,
                                          build_vertex_to_tuples,
                                          build_edge_to_tuples)
    from hymeko_neuro.hyperedge.signedkan import (MultiLayerSignedKANConfig,
                             build_vertex_triad_incidence)
    from hymeko_neuro.baselines.sgcn_model import SGCN, build_signed_adj
    ...
    if dataset.startswith("sbm_n"):
        ...
        from hymeko_neuro.data.datasets import (build_polyhedral_mesh,
    ...
    # ── Label-shuffle sanity check (ChatGPT audit, 2026-05-14). ────
    # Graph-level shuffle: permute the signs on training edges. This
    # affects (a) the supervised target y_tr and (b) the cycle
    # σ-products that consume g.signs. Test edges are UNTOUCHED so
    # AUC remains meaningful as a measure of "can the model recover
    # test signs from structural patterns + shuffled training
    # signal?". A clean architecture should produce AUC ≈ 0.5.
    # Persistent AUC > 0.5 would indicate either:
    #   - structural shortcut from test-edge participation in σ-products
    #     (the known transductive-σ-leakage convention), or
    #   - a pipeline bug somewhere we'd want to find.
    if shuffle_train_signs:
        shuffle_rng = np.random.default_rng(seed + 100003)  # distinct stream
        perm = shuffle_rng.permutation(len(tr_idx))
        # Apply in place to g.signs at the train indices, so cycle
        # enumeration downstream sees the shuffled view.
        g.signs[tr_idx] = g.signs[tr_idx][perm]
        print(f"[run_final_cell] LABEL-SHUFFLE active: permuted signs on "
              f"{len(tr_idx)} training edges (seed+100003="
              f"{seed + 100003}).", flush=True)
    ...
    from hymeko_neuro.hyperedge.n_tuples import construct_2  # noqa: F401  (cached_construct_2 is the live path)
    # ── Tuple-spec parsing ──────────────────────────────────────────
    #
    ...
    if any(spec[0] == "walk" for spec in tuple_specs):
        from hymeko_neuro.hyperedge.walks import construct_walks  # noqa: F401  (cached_construct_walks is the live path)
    ...
    if use_arc_weights:
        from hymeko_neuro.data.datasets.continuous import load_continuous
        from hymeko_neuro.hyperedge.arc_weights import (
    ...
    if n_branches > 1:
        ...
        from hymeko_neuro.hyperedge.side_signedkan import (
    ...
    entropy_reg = None
    if entropy_lambda > 0:
        from hymeko_neuro.hyperedge.entropy_reg import EntropyRegulariser, EntropyRegConfig
    ...
    if is_slashdot:
        ...
        out = dict(dataset=dataset, model="HSiKAN-mixed", hidden=hidden,
                      n_test=int(len(te_idx)),
        ...
        if emit_full_metrics:
            from hymeko_neuro.experiments.eval.eval_metrics_full import full_binary_metrics
...
def cell_scene(hidden: int, n_epochs: int, device):
    from hymeko_neuro.data.adapters.visual_genome import (synth_dataset,
                                            edge_features_from_bboxes)
    from hymeko_neuro.graph.cycle_cache import cached_construct_2
    from hymeko_neuro.data.datasets import SignedGraph
    from hymeko_neuro.hyperedge.hyperedges import construct  # noqa: F401  (compat)
    from hymeko_neuro.hyperedge.n_tuples import construct_2  # noqa: F401  (cached_construct_2 is live)
    from hymeko_neuro.models.mixed_arity_signedkan import (MixedAritySignedKAN,
                                          MixedAritySignedKANConfig,
                                          build_edge_to_tuples)
    from hymeko_neuro.hyperedge.signedkan import (MultiLayerSignedKANConfig,
...
def main():
    ...
    ap.add_argument("--save-checkpoint", default=None,
                    help="Path to write a demo checkpoint (state_dict + cfg + "
                         ...
                         "hymeko_neuro.experiments.demo.gui. Only signed-graph datasets "
                         "(bitcoin*/slashdot/epinions/sbm/...).")
    ap.add_argument("--shuffle-train-signs", action="store_true",
                    help="LABEL-SHUFFLE sanity check (ChatGPT audit). Permutes "
                         "the signs on training edges (graph-level — affects "
                         "y_tr AND cycle σ-products); test edges untouched. "
                         "Expected on a clean architecture: AUC ≈ 0.5.")
    ap.add_argument("--emit-full-metrics", action="store_true",
                    help="Augment the output JSON row with full per-class "
                         "metrics (accuracy, precision/recall per class, "
                         "macro-F1, etc.) via the eval_metrics_full helper. "
                         "Adds keys: accuracy, precision_pos, recall_pos, "
                         "precision_neg, recall_neg, f1_pos, f1_neg, f1_macro_full. "
                         "Used by the Nature Comm head-to-head SE-SGformer "
                         "accuracy comparison (2026-05-17 rescore).")
    args = ap.parse_args()
