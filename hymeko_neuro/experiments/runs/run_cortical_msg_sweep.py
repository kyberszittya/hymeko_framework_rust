...
:mod:`hymeko_neuro.experiments.cortical_pgraph_mapping`, and optionally
runs the Slice-1 cortical benchmark on each selection.

Use::

    PYTHONPATH=$PWD python -m \\
        hymeko_neuro.experiments.runs.run_cortical_msg_sweep \\
...
def _resolve_binning(name: str):
    ...
    :class:`BinningConfig`."""
    from hymeko_neuro.experiments.cortical import BinningConfig
    if name == "shallow":
...
def _build_backbone(kind: str, *, image_size: int, in_channels: int,
    ...
    * `kind == "gomb"`: hypergraph-machine `RicciStimBackbone`
    ...
    """
    from hymeko_neuro.experiments.cortical import (
...
def main() -> None:
    ...
    from hymeko_neuro.experiments.cortical_pgraph_mapping import (
