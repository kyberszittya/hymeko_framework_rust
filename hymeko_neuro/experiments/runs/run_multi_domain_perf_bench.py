...
Running:
    HSIKAN_TORCH_COMPILE=1 python -m hymeko_neuro.experiments.runs.run_multi_domain_perf_bench

Output:
    hymeko_neuro/experiments/results/multi_domain_perf_bench.json
"""
...
from hymeko_neuro.data.datasets import SignedGraph, load, deduplicate_pairs, split
from hymeko_neuro.data.datasets import sbm_signed, hierarchical_signed
from hymeko_neuro.hyperedge.hyperedges import construct
from hymeko_neuro.hyperedge.n_tuples import construct_k, construct_2
...
def domain_scene(device, n_epochs=30) -> list[dict]:
    ...
    from hymeko_neuro.data.adapters.visual_genome import (
...
def domain_bitcoin_summary() -> list[dict]:
    p = Path("hymeko_neuro/experiments/results/inference_bench.json")
    if not p.exists():
...
def main():
    ...
    out_path = Path("hymeko_neuro/experiments/results/multi_domain_perf_bench.json")
