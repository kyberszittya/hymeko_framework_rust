...
One file, mode arg (§6.5 #13):
    python -m hymeko_neuro.experiments.runs.run_no_leak_benchmark --smoke
    python -m hymeko_neuro.experiments.runs.run_no_leak_benchmark --full
"""
...
_BASELINE_AUDIT = "hymeko_neuro.experiments.runs.run_baseline_audit"
...
SUBPROC_RUNNERS: dict[str, Runner] = {
    ...
    "gomb": Runner(
        "hymeko_neuro.experiments.runs.run_gomb_smoke", "val_auroc",
...
def main(
    ...
    out_path = Path(out_path) if out_path else Path(
        f"hymeko_neuro/experiments/results/no_leak_{label}.jsonl")
