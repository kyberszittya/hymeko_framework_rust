...
:mod:`hymeko_neuro.experiments.hsikan_pgraph_mapping`, and (optionally)
launches one HSIKAN training run per ABB / SSG selection.

Use::

    PYTHONPATH=$PWD python -m hymeko_neuro.experiments.runs.run_hsikan_msg_sweep \\
...
def run_pgraph_dump(
    ...
    # HSiKAN sweeps use the no-excess regime deliberately; the engine
    # defaults to canonical since 2026-05-27, so request it explicitly
    # unless --relaxed-msg.
    if relaxed_msg:
...
def _map_one(unit_names: list[str], dataset: str, seed: int, base: dict[str, Any]) -> dict[str, Any]:
    from hymeko_neuro.experiments.hsikan_pgraph_mapping import (
...
def main() -> None:
    ...
    for sel in selections:
        for seed in args.seeds:
            ...
            if args.train:
                try:
                    from hymeko_neuro.experiments.runs.run_compare import run_one
