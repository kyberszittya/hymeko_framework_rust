...
Example::
    ...
    PYTHONPATH=$PWD python -m hymeko_neuro.experiments.runs.run_gomb_msg_sweep \\
...
from hymeko_neuro.experiments.gomb_pgraph_mapping import (
    merge_structure_knobs,
    smoke_argv_from_knobs,
)
from hymeko_neuro.hymeko.hymeko_driver import parse_training


def _repo_root() -> Path:
    # parents[0] = experiments/runs, parents[1] = experiments,
    # parents[2] = hymeko_neuro, parents[3] = repo root (where
    # the workspace `Cargo.toml`, `target/`, and `data/hsikan/...`
    # live). Phase-9 fix (2026-05-19): previously returned [2],
    # which pointed at `hymeko_neuro/` — `hymeko_neuro/assets/data/`
