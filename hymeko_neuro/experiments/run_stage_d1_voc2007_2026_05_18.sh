...
OUT_DIR="hymeko_neuro/experiments/results/stage_d1_voc2007_${STAMP}"
mkdir -p "$OUT_DIR"
...
# Queue behind any in-flight hymeko_neuro training (including the
# IMDB arch-fairness run launched alongside this orchestrator).
echo "[orch] waiting for GPU..." | tee -a "$LOG"
while pgrep -af 'hymeko_neuro\.src\.(run_optuna_search|run_final_cell|run_gomb_smoke|vision\.train|sequence\.(train|run))' \
...
systemd-run --user --scope --quiet \
  ...
  python -u -m hymeko_neuro.experiments.vision.train_voc_stagec \
