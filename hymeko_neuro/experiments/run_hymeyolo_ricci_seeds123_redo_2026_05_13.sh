...
#
# This script:
#   - queues behind any in-flight hymeko_neuro.experiments.runs.run_optuna_search or
...
# Use the env where torch lives; hymeko_neuro imports work from either.
...
# Wait for the GPU to be free of hymeko_neuro processes.
echo "[$(date -Is)] waiting for run_optuna_search / run_final_cell to clear..." | tee -a "$MASTER"
SELF_PID="$$"
while pgrep -af 'hymeko_neuro\.src\.run_optuna_search|hymeko_neuro\.src\.run_final_cell' \
...
run_seed() {
  ...
  local cmd=(python -m hymeko_neuro.experiments.vision.train_circles_ricci
