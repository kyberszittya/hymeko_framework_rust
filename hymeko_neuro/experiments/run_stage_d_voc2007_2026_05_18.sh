...
#      Budget: ~6h GPU.
#
# Queues behind any in-flight hymeko_neuro training (HSiKAN rescore,
...
OUT_DIR="hymeko_neuro/experiments/results/stage_d_voc2007_${STAMP}"
mkdir -p "$OUT_DIR"
...
echo "[orch] waiting for GPU (hymeko_neuro training)..." | tee -a "$LOG"
while pgrep -af 'hymeko_neuro\.src\.(run_optuna_search|run_final_cell|run_gomb_smoke|vision\.train)' \
