...
#      Budget: ~2 h GPU.
#
# Queues behind any in-flight hymeko_neuro training (HyMeYOLO
...
OUT_DIR="hymeko_neuro/experiments/results/seq_hsikan_imdb_${STAMP}"
mkdir -p "$OUT_DIR"
...
while pgrep -af 'hymeko_neuro\.src\.(run_optuna_search|run_final_cell|run_gomb_smoke|vision\.train_circles_ricci|vision\.train_voc)' \
...
systemd-run --user --scope --quiet \
  ...
  python -u -m hymeko_neuro.experiments.sequence.train_imdb_classifier \
