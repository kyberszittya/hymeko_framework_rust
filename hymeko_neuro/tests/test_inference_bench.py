...
model stack, so they skip cleanly when torch is unavailable. Run with::
    ...
    uv run --group ml --group dev pytest -p no:randomly \
        hymeko_neuro/tests/test_inference_bench.py
"""
...
from hymeko_neuro.experiments.runs.run_inference_bench import (
...
def test_main_renders_from_json(tmp_path):
    from hymeko_neuro.experiments.runs import bench_to_png
