...
from hymeko_neuro.models.hymeko_gomb import (
...
from hymeko_neuro.models.hymeko_gomb.joint_enumeration import (
    JOINT_BA_SLOTS, build_joint_ba_pools,
)
from hymeko_neuro.models.hymeko_gomb.shells import scatter_mean
...
def test_gomb_trains_on_synthetic_moons():
    ...
    from hymeko_neuro.data.datasets import make_moon_signed_graph
...
def test_joint_mix_cuda_route_peak_memory_strictly_below_pyramid():
    ...
    """
    from hymeko_neuro.experiments.runs.run_gomb_smoke import _degree_to_tier
...
def test_joint_mix_gomb_bitcoin_otc_val_auc_regression():
    ...
    """
    from hymeko_neuro.data.datasets import load
    from hymeko_neuro.experiments.runs.run_gomb_smoke import _degree_to_tier, _train_val_split
...
def test_hymeko_gomb_bitcoin_otc_val_auc_cuda():
    ...
    """
    from hymeko_neuro.data.datasets import load
    from hymeko_neuro.experiments.runs.run_gomb_smoke import (
        _degree_to_tier,
        _enumerate_cycles,
        _train_val_split,
    )
...
def test_sample_params_compact_is_narrow():
    from hymeko_neuro.experiments.runs.run_gomb_tune import sample_params
...
def test_for_joint_mix_tuning_clears_cycle_ks_and_samples_walks():
    from hymeko_neuro.experiments.runs.run_gomb_tune import for_joint_mix_tuning, sample_params
...
def test_for_joint_mix_clamps_topk_on_bitcoin_wide():
    from hymeko_neuro.experiments.runs.run_gomb_tune import for_joint_mix_tuning
...
def test_tuner_objective_val_vs_test():
    from hymeko_neuro.experiments.runs.run_gomb_tune import _tuner_objective
...
def test_n_params_from_row():
    from hymeko_neuro.experiments.runs.run_gomb_tune import _n_params_from_row
...
def test_subsample_joint_pools_reduces_rows():
    from hymeko_neuro.experiments.runs.run_gomb_smoke import _subsample_joint_pools
...
def test_build_cmd_joint_mix_includes_cli_flags():
    ...
    from hymeko_neuro.experiments.runs.run_gomb_tune import _build_cmd
...
def test_run_gomb_smoke_json_includes_inference_fields():
    ...
    cmd = [
        sys.executable,
        "-m",
        "hymeko_neuro.experiments.runs.run_gomb_smoke",
        "--dataset",
        "sbm_n200",
        "--n-epochs",
        "1",
...
def test_parse_last_gomb_json_line():
    from hymeko_neuro.experiments.runs.run_gomb_tune import _parse_last_gomb_json
...
def test_heldout_edge_metrics_label_keys():
    ...
    from hymeko_neuro.experiments.runs.run_gomb_smoke import _heldout_edge_metrics
