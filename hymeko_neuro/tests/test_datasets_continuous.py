...
def test_weighted_signed_graph_dataclass_signs_property():
    from hymeko_neuro.data.datasets import WeightedSignedGraph
...
def test_continuous_bitcoin_alpha_preserves_magnitude(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hymeko_neuro.data.datasets.DATA_DIR", tmp_path)
    monkeypatch.setattr("hymeko_neuro.data.datasets.legacy.DATA_DIR", tmp_path)
    monkeypatch.setattr("hymeko_neuro.data.datasets.DATA_DIR", tmp_path)
    monkeypatch.setattr("hymeko_neuro.data.datasets.legacy.DATA_DIR", tmp_path)
    from hymeko_neuro.data.datasets import load_continuous
...
def test_continuous_loader_rejects_unsupported_dataset_format(monkeypatch):
    ...
    monkeypatch.setitem(
        __import__("hymeko_neuro.data.datasets", fromlist=["FORMATS"]).FORMATS,
