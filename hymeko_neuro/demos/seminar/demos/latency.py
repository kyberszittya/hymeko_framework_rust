...
``python -m hymeko_neuro.experiments.runs.run_inference_bench`` (heavy, not a
...
DEFAULT_JSON = (
    REPO_ROOT / "hymeko_neuro" / "experiments" / "results" / "inference_bench.json"
)
    ...
    def run(self, args: argparse.Namespace, ctx: DemoContext) -> DemoResult:
        ...
        if not json_path.is_file():
            raise FileNotFoundError(
                ...
                f"`python -m hymeko_neuro.experiments.runs.run_inference_bench`."
            )
