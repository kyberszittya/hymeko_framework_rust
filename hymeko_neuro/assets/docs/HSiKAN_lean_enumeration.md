...
exercised in `hymeko_neuro/experiments/run_overnight_abb_validation_2026_05_11.sh`.
...
`python -m hymeko_neuro.run_hsikan_lean_profile` runs `run_final_cell` in
...
export PYTHONPATH=.
uv run python -m hymeko_neuro.run_hsikan_lean_profile --help
PY="$(uv run python -c "import sys, torch; print(sys.executable)")"
PYTHONPATH=. uv run python -m hymeko_neuro.run_hsikan_lean_profile \
  --python "${PY}" \
  --datasets bitcoin_alpha --seeds 0 --hidden 8 --profiles clean_baseline \
  --n-epochs 1 --timeout-s 3600 --out /tmp/hsikan_smoke.jsonl
```

Lint / tests: `uv run ruff check hymeko_neuro/`, `uv run pytest -p no:randomly hymeko_neuro/tests/test_hsikan_lean_profile.py`.
...
PYTHONPATH=. "${PY}" -m hymeko_neuro.run_hsikan_lean_profile \
  --python "${PY}" \
  ...
```
...
The shell helper `hymeko_neuro/experiments/run_hsikan_lean_bitcoin.sh` sets
...
"${PY}" -m hymeko_neuro.run_hsikan_lean_profile \
  --python "${PY}" \
...
Shell wrapper: `hymeko_neuro/experiments/run_hsikan_lean_bitcoin.sh`.
