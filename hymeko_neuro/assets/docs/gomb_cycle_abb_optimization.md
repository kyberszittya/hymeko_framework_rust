...
From the repo root (set `PYTHONPATH` as for other `hymeko_neuro` CLIs):

```bash
python -m hymeko_neuro.eval.benchmarks.run_gomb_cycle_abb_compare \
  --dataset bitcoin_otc \
  --edge-split 80_10_10 \
  --device cpu \
  --seed 0 \
...
python -m hymeko_neuro.eval.benchmarks.run_gomb_cycle_abb_compare \
  ...same flags... \
  --jsonl-out hymeko_neuro/experiments/results/gomb_abb_compare.jsonl
```
...
- **P-graph MSG / SSG / ABB** (outer discrete structures): run
  ``python -m hymeko_neuro.run_gomb_msg_sweep --pgraph data/hsikan/sweep_msg_gomb.hymeko``
  or ``hymeko_driver --backend gomb`` (see ``hymeko_neuro/hymeko/hymeko_driver.py``).
...
- **Key-art image prompt** (neo-Tokyo + repo-backed HUD numbers):  
  `hymeko_neuro/assets/docs/gomb_key_art_prompt.md`.
