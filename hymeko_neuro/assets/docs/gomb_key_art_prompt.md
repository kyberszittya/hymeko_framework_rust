...
| Claim | Value | Where |
|--------|--------|--------|
| HSiKAN test AUC (published mean ± std) | **Bitcoin Alpha 0.939 ± 0.011**, **OTC 0.930 ± 0.008** | `hymeko_neuro/eval/benchmarks/sota_reference.json` |
...
| HSiKAN Optuna chase OTC (`run_final_cell` JSON **`auc`**, test split, **seed 0 only**) | trial **1**: **0.9945** (**99.45%**) — chase vs SiGAT bar **0.934**; **not** the 5-seed headline row | Study `hymeko_neuro/experiments/results/optuna_otc_sigat_chase_2026_05_12.db`; default **transductive** tuple protocol (`HSIKAN_STRICT_PROTOCOL` off). Re-verify from study / chase stdout before print. |
