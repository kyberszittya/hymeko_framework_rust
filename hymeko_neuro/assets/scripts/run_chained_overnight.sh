...
#
# Usage:
#   nohup bash hymeko_neuro/assets/scripts/run_chained_overnight.sh \
#         > /tmp/chained_overnight.log 2>&1 &
set -u

CR_FILE="hymeko_neuro/experiments/results/overnight_camera_ready.jsonl"
EXPECTED_CELLS=75
...
while true; do
  ...
  if [[ "$age" -gt 300 ]]; then
    ...
    if ! pgrep -f "hymeko_neuro\.src\.run_final_cell" >/dev/null 2>&1; then
