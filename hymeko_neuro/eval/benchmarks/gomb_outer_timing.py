...
Run from repo root (default **CUDA** when ``torch.cuda.is_available()``, else CPU)::
    ...
    python -m hymeko_neuro.eval.benchmarks.gomb_outer_timing
    python -m hymeko_neuro.eval.benchmarks.gomb_outer_timing --iters 40
    python -m hymeko_neuro.eval.benchmarks.gomb_outer_timing --device cpu
...
Optional **torch.compile** on a copy of the shell (CUDA recommended; extra warmup)::
    ...
    python -m hymeko_neuro.eval.benchmarks.gomb_outer_timing --torch-compile \\
...
(including per-shell breakdown), train with::
    ...
    python -m hymeko_neuro.experiments.runs.run_gomb_smoke --dataset bitcoin_otc --n-epochs 50
...
from hymeko_neuro.models.hymeko_gomb.shells import OuterFIRShell
from hymeko_neuro.experiments.runs.run_gomb_smoke import _enumerate_cycles, _train_val_split
