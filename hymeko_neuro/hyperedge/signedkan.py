...
class SignedKANLayer(nn.Module):
    ...
    def __init__(self, cfg: SignedKANConfig):
        ...
        if self.inner_skip == "highway":
            ...
            self.gate_proj = None
            self.gate_coef = None
            self.gate_W_arc = None
        elif self.inner_skip == "cr_highway":
            # Catmull-Rom-parameterised highway gate.  Same broadcast
            # over the σ-branches as the existing ``highway`` mode
            # (gate shape (T, k, d) → broadcast over S), but the
            # sigmoid(Linear) is replaced by a per-channel CR spline
            # whose coefficients are perturbed per-edge by the arc
            # weight.  CR is linear in its coefs, so the perturbation
            # is implemented as one extra CR eval rather than a
            # (T, k, d, G) effective-coef tensor.
            G = max(int(getattr(cfg, "grid", 5)), 4)   # CR needs ≥ 4 knots
            self._cr_grid = G
            self.gate_proj = nn.Linear(d, d)
            # Base CR coefs: bias the output low (≈ -2) so the
            # sigmoid maps to ≈ 0.12 at init — matches the existing
            # highway bias.fill_(-2.0).
            self.gate_coef = nn.Parameter(
                torch.full((d, G), -2.0, dtype=torch.float32),
            )
            # Arc-weight perturbation of the CR coefs.  Init at zero
            # → at start the new mode is exactly a CR-only gate
            # (arc weight is ignored).  The model learns to lift
            # this away from zero only if arc weights actually help.
            self.gate_W_arc = nn.Parameter(
                torch.zeros((d, G), dtype=torch.float32),
            )
            with torch.no_grad():
    ...
    def _can_use_triton_inner(self, x: torch.Tensor) -> bool:
        ...
        in hymeko_neuro/tests/test_triton_kernels.py.
    ...
    def _triton_inner_agg(
        ...
        PyTorch path."""
        from hymeko_neuro.kernels.triton_kernels import (
    ...
    def forward(self, x: torch.Tensor,
                ...
                arc_weights: torch.Tensor | None = None,
               ) -> torch.Tensor:
        ...
        if chunk_t > 0 and triad_v.shape[0] > chunk_t:
            ...
            for s in range(0, triad_v.shape[0], chunk_t):
                ...
                w_chunk = (arc_weights[s:e] if arc_weights is not None
                            else None)
    ...
    def _forward_impl(self, x: torch.Tensor,
        ...
        # hymeko_neuro/tests/test_triton_kernels.py).  Set
        ...
        if self._can_use_triton_inner(x):
            ...
            if self.inner_skip == "residual":
                ...
                # CR-parameterised highway gate with per-edge arc-
                # weight perturbation. Same broadcast over σ as the
                # plain ``highway`` mode.
                from .splines import _catmull_rom_eval
                G_gate = self._cr_grid
                # Project h_v to a tanh-bounded per-channel input.
                x_proj = torch.tanh(self.gate_proj(h_v))        # (T, k, d) ∈ [-1, 1]
                # Base CR evaluation (per-channel coefs (d, G)
                # broadcast to (T*k, d) inputs).
                base_coef = self.gate_coef.unsqueeze(0).unsqueeze(0)  # (1, 1, d, G)
                base_coef = base_coef.expand(T, k, -1, -1)            # (T, k, d, G)
                logit_base = _catmull_rom_eval(
                    base_coef.reshape(-1, d, G_gate),
                    x_proj.reshape(-1, d),
                    G_gate,
                ).reshape(T, k, d)
                if (arc_weights is not None
                        and self.gate_W_arc is not None):
                    # CR is linear in its coefs, so the perturbed
                    # eval = base eval + w·CR(W_arc, x).
                    pert_coef = self.gate_W_arc.unsqueeze(0).unsqueeze(0)  # (1,1,d,G)
                    pert_coef = pert_coef.expand(T, k, -1, -1)
                    logit_pert = _catmull_rom_eval(
                        pert_coef.reshape(-1, d, G_gate),
                        x_proj.reshape(-1, d),
                        G_gate,
                    ).reshape(T, k, d)
                    w_b = arc_weights.unsqueeze(-1).to(x.dtype)   # (T, k, 1)
                    logit = logit_base + w_b * logit_pert
                else:
                    logit = logit_base
                T_inner = torch.sigmoid(logit)                  # (T, k, d)
                T_inner_b = T_inner.unsqueeze(2)                # (T, k, 1, d)
                x_b = h_v.unsqueeze(2)
                inner_all = T_inner_b * inner_all + (1.0 - T_inner_b) * x_b
...
class SignedKAN(nn.Module):
    ...
    def encode_triads(self, triad_v: torch.Tensor,
        ...
        ``arc_weights`` (optional, ``(T, k)``): per-vertex normalised
        arc weights threaded into the layer's ``cr_highway`` gate.
        Ignored when the layer is not in ``cr_highway`` mode.
        """
        x = self.node_embed.weight     # (n_nodes, d)
        h_t = self.layer(x, triad_v, triad_sigma, arc_weights=arc_weights)
...
class MultiLayerSignedKAN(nn.Module):
    ...
    def encode_triads(self, triad_v: torch.Tensor,
                      ...
                      return_h_v: bool = False,
                      arc_weights: torch.Tensor | None = None,
                      initial_h_v: torch.Tensor | None = None):
        ...
        ``arc_weights``: optional per-vertex normalised arc weights of
        shape ``(T, k)``, threaded through to each layer's
        ``cr_highway`` gate (no-op when ``inner_skip != "cr_highway"``).

        ``initial_h_v``: optional ``(V, d)`` tensor that overrides
        ``self.node_embed.weight`` as the starting vertex embedding.
        Used by external callers (e.g. Gömb's stacked-middle path) to
        inject their own pre-computed per-vertex features instead of
        the stack's built-in embedding. When None, falls back to the
        built-in embedding for backward compat.
        """
        if initial_h_v is not None:
            h_v = initial_h_v                            # external h_v
        else:
