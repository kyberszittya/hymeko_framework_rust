...
Unit tests in `hymeko_neuro/tests/test_cpml.py` validate:
...
class CPML(nn.Module):
    ...
    def _edge_logits_simple(self, x_final: torch.Tensor,
                              edges_to_score: torch.Tensor
                              ) -> torch.Tensor:
        """Factored-matmul head, used directly when E is small or as
        the per-chunk function for the batched-checkpoint path."""
    ...
    def _edge_logits(self, x_final: torch.Tensor,
        ...
        """Factored-matmul edge head, with edge-batching + per-chunk
        grad-checkpoint when ``E`` is large.

        Memory layers:
        - ``Linear(2d, h)(cat([u, v])) ≡ u @ W[..., :d].T +
          v @ W[..., d:].T + b`` (linear-algebra identity, ≤4e-7).
          Avoids the (E, 2d) cat intermediate. Bit-identical.
        - For ``E > _edge_logits_batch`` (default 200k), process in
          chunks under ``torch.utils.checkpoint.checkpoint`` so the
          (E_chunk, d) index tensors and per-chunk activations are
          freed between chunks during forward; backward recomputes
          per chunk. Peak edge-head memory falls from
          O(E · d_final) to O(E_chunk · d_final) — the right scaling
          for Epinions (640k train edges) and similarly large
          datasets.
        """
        E = edges_to_score.shape[0]
        batch = getattr(self, "_edge_logits_batch", 200_000)
        # Bypass batching when small, or in inference (no graph to
        # checkpoint anyway).
        if E <= batch or not (self.training and torch.is_grad_enabled()):
            return self._edge_logits_simple(x_final, edges_to_score)
        out_chunks: list[torch.Tensor] = []
        for s in range(0, E, batch):
            end = min(s + batch, E)
            edges_chunk = edges_to_score[s:end]
            out_chunk = torch.utils.checkpoint.checkpoint(
                self._edge_logits_simple, x_final, edges_chunk,
                use_reentrant=False,
            )
