...
Moved here from ``hymeko_neuro/`` flat layout on 2026-05-19 as
...
external code that uses ``from hymeko_neuro.hyperedge import X``
keeps working unchanged. Pre-Slice-F code that wrote
``from hymeko_neuro.<name> import X`` has been migrated to use
``from hymeko_neuro.hyperedge.<name> import X`` (or via the package
namespace).
