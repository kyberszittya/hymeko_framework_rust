...
```bash
PYTHONPATH=. python hymeko_neuro/examples/pose_demo.py
```
...
```bash
PYTHONPATH=. python hymeko_neuro/examples/pose_demo.py --force-retrain
```

Higher-fidelity training (matches the gate-passing config):

```bash
PYTHONPATH=. python hymeko_neuro/examples/pose_demo.py --force-retrain --n-epochs 60
```
...
- Model implementation: [`hymeko_neuro/experiments/vision/fuzzy_pose.py`](../src/vision/fuzzy_pose.py)
- Tests: [`hymeko_neuro/tests/test_fuzzy_pose.py`](../tests/test_fuzzy_pose.py)
