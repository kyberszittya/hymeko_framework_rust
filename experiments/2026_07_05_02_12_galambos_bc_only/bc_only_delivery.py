...
from hymeko_rl.experiments.galambos_demo import ShepherdDemonstrator
from hymeko_rl.train.bc import behaviour_clone
from hymeko_rl.train.campaign import tee_stdout

DIFFICULTY = 0.3
N_DEMOS = 200
BC_EPOCHS = 100      # right-sized for the 27.5k shepherd corpus (old 200 was tuned on 3.2k; still 4x updates)
BC_BATCH = 512       # measured (2026-07-05): cuda b512 = 0.70 ep/s vs cpu b128 = 0.18 (b128 is launch-bound on GPU)
BC_DEVICE = "auto"
SEEDS = (0, 1, 2)
N_EVAL = 50
EVAL_SEED = 9_000
...
def main() -> int:
    ...
    with tee_stdout(exp / "run.log"):
        ...
        for s in SEEDS:
            ...
            losses = behaviour_clone(actor, obs, acts, n_epochs=BC_EPOCHS, batch=BC_BATCH,
                                     device=BC_DEVICE, seed=s)
        ...
        summary = {"demonstrator_delivery": round(demo_rate, 3), "bc_delivery_median": dels[len(dels) // 2],
                   ...
                   "config": {"difficulty": DIFFICULTY, "n_demos": N_DEMOS, "bc_epochs": BC_EPOCHS,
                              "bc_batch": BC_BATCH, "bc_device": BC_DEVICE, "teacher": "shepherd (declarative)",
