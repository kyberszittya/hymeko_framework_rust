use crate::service::{HymekoDaemon, PublishRequest};
...
use tracing::{debug, error, info, warn}; // Import the struct from the service module
...
impl HymekoDaemon {
        ...
        rayon::spawn(move || {
            match serde_cbor::from_slice::<CborPayload>(&payload) {
                Ok(cbor_payload) => {
                    ...
                    let extractor = ScalarWeightExtractor;
                    ...
                    if let Err(e) = tx_pub.blocking_send(req) {
                        ...
                        let _ = tx.send(Err(
                            format!("Failed to send to publisher actor: {}", e).into()
                        ));
                        return;
                    }
    ...
    async fn execute_compilation(
        ...
        rayon::spawn(move || {
            ...
            let owned_ir = store
                .take_last_ir()
                .expect("Failed to extract IR from ephemeral compiler");
        ...
        match rx.await {
            Ok(result) => {
                ...
                match &result {
                    Ok(_) => {
                        info!(request_id, service = %self.config.service_name, source = "execute_compilation", elapsed_ms, "compilation pipeline completed")
                    }
                    Err(e) => {
                        error!(request_id, service = %self.config.service_name, source = "execute_compilation", elapsed_ms, error = %e, "compilation pipeline failed")
                    }
                }
    ...
    pub async fn handle_utf8_query(
        &self,
        payload: Vec<u8>,
    ...
    pub async fn handle_cbor_query(
        &self,
        payload: Vec<u8>,
        tx: mpsc::Sender<PublishRequest>,
    ) -> Result<(), Box<dyn Error>> {
    ...
    pub fn compile_to_ir_only(
        &self,
        dsl_source: String,
    ) -> Result<Arc<Ir>, Box<dyn Error + Send + Sync>> {
    ...
    pub fn expand_graph_clique(&self, ir: &Ir) -> Vec<u8> {
        let etag = ir.doc_hash.unwrap_or(HashId([0; 32])).0;
        ...
        let extractor = ScalarWeightExtractor;
        let hg_view =
            HyperGraphView::<f32, EdgeWScalar<f32>, f32>::from_ir(ir, &self.agg_cfg, &extractor);

        // Using your clique expansion algorithm here
        let tensor = hymeko_hre::expansion::clique_expansion_coo::<_, _, f32>(&hg_view);

        tensor_to_arrow_bytes(tensor, &etag, &graph_name)
            .expect("Failed to encode Clique Arrow IPC stream")
    }
    ...
    pub fn expand_graph(&self, ir: &Ir) -> Vec<u8> {
        let etag = ir
            ...
            .unwrap_or_else(|| {
                warn!(
                    marker = "[-]",
                    "IR reached execution without doc_hash. Using zeroed fallback."
                );
        ...
        tensor_to_arrow_bytes(tensor, &etag, graph_name.as_str())
            .expect("Failed to encode Arrow IPC stream")
    }
}
