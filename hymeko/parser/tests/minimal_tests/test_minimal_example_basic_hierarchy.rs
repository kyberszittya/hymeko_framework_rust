...
fn parses_minimal_example_context_fields() {
    ...
    assert_eq!(as_node(&lev0_body[0]).inner.name, "node0");
    assert_eq!(as_node(&lev0_body[1]).inner.name, "node1");
    assert_eq!(as_node(&lev0_body[2]).inner.name, "node2");
    assert_eq!(as_node(&lev0_body[3]).inner.name, "node3");

    // node0 is a block and contains: node0;
    let node0_block = as_node(&lev0_body[0]);
    let node0_body = body(node0_block);
    assert_eq!(node0_body.len(), 1);
    let inner_node0 = as_node(&node0_body[0]);
    ...
    assert_eq!(as_node(&lev1_body[0]).inner.name, "node0");
}