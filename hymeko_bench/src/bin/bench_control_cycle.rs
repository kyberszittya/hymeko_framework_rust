...
//!
//! Measures wall-clock for one grasping-scenario control cycle across three
//! pipelines (hypergraph HGNN head, flat TD3 MLP, PID+TD3 blend). Stages:
//! state assembly, forward, decode, total. Weights are deterministic noise;
//! this is a compute-complexity comparison, not a trained-policy benchmark.
//!
//! Emits `hymeko_bench/results/control_cycle.csv`. Run:
...
type Td3RawTimings = (Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>);
...
struct HgPipeline {
    ...
    w1: Vec<Vec<f32>>,
    b1: Vec<f32>, // [H_MLP x F_HG], [H_MLP]
    w2: Vec<Vec<f32>>,
    b2: Vec<f32>, // [ACTION_DIM x H_MLP], [ACTION_DIM]
    // Scratch buffers.
...
impl HgPipeline {
    ...
    fn param_count(&self) -> usize {
        self.edge_w.len()
            + self.embed.len()
            + self.w1.iter().map(|r| r.len()).sum::<usize>()
            + self.b1.len()
            + self.w2.iter().map(|r| r.len()).sum::<usize>()
            + self.b2.len()
    }
    ...
    fn state_assembly(&mut self, sensors: &[f32]) {
        ...
        for i in 0..n_v {
            self.node_x[i] = sensors[i % sensors.len()];
        }
    }
    ...
    fn decode(&self, vel: &mut [f32]) {
        for (v, a) in vel.iter_mut().zip(&self.action) {
            *v = a.tanh();
        }
    }
}
...
struct Td3Pipeline {
    w1: Vec<Vec<f32>>,
    b1: Vec<f32>, // [H_MLP x STATE_DIM]
    w2: Vec<Vec<f32>>,
    b2: Vec<f32>, // [H_MLP x H_MLP]
    w3: Vec<Vec<f32>>,
    b3: Vec<f32>, // [ACTION_DIM x H_MLP]
    state: Vec<f32>,
    h1: Vec<f32>,
    h2: Vec<f32>,
    action: Vec<f32>,
...
impl Td3Pipeline {
    fn new(rng: &mut Rng) -> Self {
        Self {
            w1: rng.mat(H_MLP, STATE_DIM),
            b1: rng.vec(H_MLP),
            w2: rng.mat(H_MLP, H_MLP),
            b2: rng.vec(H_MLP),
            w3: rng.mat(ACTION_DIM, H_MLP),
            b3: rng.vec(ACTION_DIM),
    ...
    fn param_count(&self) -> usize {
        self.w1.iter().map(|r| r.len()).sum::<usize>()
            ...
            + self.w2.iter().map(|r| r.len()).sum::<usize>()
            + self.b2.len()
            + self.w3.iter().map(|r| r.len()).sum::<usize>()
            + self.b3.len()
    }
    #[inline(always)]
    fn state_assembly(&mut self, sensors: &[f32]) {
        for i in 0..STATE_DIM {
            self.state[i] = sensors[i % sensors.len()];
        }
    }
    #[inline(always)]
    fn forward(&mut self) {
...
impl PidTd3Pipeline {
    ...
    fn state_assembly(&mut self, sensors: &[f32]) {
        ...
        for (i, &measured) in sensors.iter().take(ACTION_DIM).enumerate() {
            ...
            self.pid_out[i] =
                self.kp[i] * self.err[i] + self.ki[i] * self.integral[i] + self.kd[i] * deriv;
        }
        // TD3 state: concatenated errors + full sensor window.
        for i in 0..STATE_DIM {
            self.td3_state[i] = sensors[i % sensors.len()];
        }
    }
    #[inline(always)]
    fn forward(&mut self) {
    ...
    #[inline(always)]
    fn decode(&self, vel: &mut [f32]) {
        for ((out, &pid), &td3_a) in vel
            .iter_mut()
            .zip(self.pid_out.iter())
            .zip(self.td3.action.iter())
        {
            *out = (pid + 0.2 * td3_a).tanh();
        }
    }
}
...
fn main() -> Result<()> {
    ...
    let mut td3_raw: Option<Td3RawTimings> = None;
    ...
    let hg_total = rows
        .iter()
        .find(|r| r.pipeline == "hypergraph" && r.stage == "total")
        .unwrap()
        .median_us;
    let td3_total = rows
        .iter()
        .find(|r| r.pipeline == "td3" && r.stage == "total")
        .unwrap()
        .median_us;
    let ratio = hg_total / td3_total;
    eprintln!(
        "\nAcceptance: hypergraph/td3 total ratio = {:.2}× (bound 3×, alarm >5×)",
        ratio
    );
    if ratio > 5.0 {
