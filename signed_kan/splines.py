...
after phase 3, the ``signedkan_wip`` line both use it); a parity test pins it to ``signedkan_wip``'s canonical
...
def catmull_rom(coef: torch.Tensor, x: torch.Tensor, grid: int) -> torch.Tensor:
    ...
    **The single canonical Catmull-Rom evaluator** for the whole repo: ``hymeko_rl`` (via the re-export in
    ``policy.py``) and ``signedkan_wip`` (which wraps this in its ``torch.compile`` fast-path) both call it.
    The gather form below is the ``signedkan_wip`` canonical body verbatim — generalised over arbitrary leading
...
def make_uniform_knots(grid: int, k: int, lo: float = -1.0, hi: float = 1.0) -> torch.Tensor:
    ...
    Verbatim from ``signedkan_wip`` so the B-spline basis is identical."""
...
def cox_de_boor(x: torch.Tensor, knots: torch.Tensor, k: int) -> torch.Tensor:
    ...
    Verbatim from ``signedkan_wip`` (the OTC benchmark's spline) so re-homing is bit-faithful."""
    x = x.unsqueeze(-1)
    left = knots[:-1]
    right = knots[1:]
