...
Same body as the RL line — :class:`~signed_kan.backbone.SignedKANBackbone` (CR signed message passing + highway +
weighted incidence) — but with the **signed-graph** input adapter (transductive node embeddings) and the
**edge-sign** head (:class:`~signed_kan.heads.EdgeSignHead`). The only differences from the RL model are the
