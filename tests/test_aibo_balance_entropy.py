...
def test_dynamic_stability_in_range_and_high_when_settled() -> None:
    env = ResidualTrotEnv(ResidualTrotConfig(residual_mode="leg", obs_mode="flat"), seed=0)
    env.reset(seed=0)
    s = env.dynamic_stability()
    assert -1.0 <= s <= 1.0
    assert s > 0.5                                         # a settled stance is dynamically stable (+)


def test_stability_w_defaults_off_and_adds_term() -> None:
    assert ResidualTrotConfig().stability_w == 0.0
    on = ResidualTrotEnv(ResidualTrotConfig(residual_mode="leg", obs_mode="flat", heading_mode="turn_then_walk",
                                            stability_w=1.0), seed=0)
    off = ResidualTrotEnv(ResidualTrotConfig(residual_mode="leg", obs_mode="flat", heading_mode="turn_then_walk",
                                             stability_w=0.0), seed=0)
    on.reset(seed=2)
    off.reset(seed=2)
    z = np.zeros(12, np.float32)
    _o, r_on, *_ = on.step(z)
    _o2, r_off, *_ = off.step(z)
    assert r_on > r_off                                    # stability term (stability_w·margin > 0 when stable)
