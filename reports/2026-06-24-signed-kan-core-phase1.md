# HSiKAN unification — phase 1: the `signed_kan/` core

**Date:** 2026-06-24 · **Branch:** soma-vision · **Plan:** `docs/plans/2026-06-24-unify-hsikan-signed-kan-core/`

## Summary
Phase 1 of the unification: extracted the shared signed-KAN core into a new pure-torch package `signed_kan/`,
isolated and tested. **No consumer touched** (`hymeko_rl` and `signedkan_wip` are unchanged — that is phase 2/3).
...
- `signed_kan/splines.py` — `catmull_rom`, `CatmullRomActivation` (the CR re-homed; byte-faithful to the
  parity-tested original), `EdgeActivation` base, `make_activation` (cr/relu/tanh Strategy).
- `signed_kan/backends.py` — `AggregationBackend` (ABC) + `DenseBatchedBackend` (the dense einsum path; the
  sparse/Triton backends slot in at phase 3 via the same trait).
- `signed_kan/layer.py` — `SignedKANLayer` (one conv: backend-agg → linear mix → spline → skip), the
  `HighwaySkip` (Schmidhuber gate, bias −2 = carry-dominant init), `_ResidualSkip`, skip Strategy.
- `signed_kan/backbone.py` — `SignedKANBackbone` (incidence {fixed, learned, weighted} + layer stack + pool).
- `signed_kan/__init__.py`, `signed_kan/tests/test_core.py`.
...
## Test results
`signed_kan/tests/test_core.py`: **11 passed** (9.0 s). Covers: dense-backend aggregation correctness +
batched-shape guard; **Catmull-Rom parity vs `signedkan_wip._catmull_rom_eval`**; skip modes (none/residual/
...
both `signed_kan/` and `hymeko_rl/policy.py`; phase 2 deletes the `policy.py` copy (strangler-fig migration). The
...
- `_SignedConv` and the old `HSiKANBackbone` body **deleted**; `_catmull_rom` / `CatmullRomActivation` are now
  **re-exports** from `signed_kan.splines` (so the Triton `cr_kernel` + parity tests keep their import path; the
  implementation lives once, in the core). The transient phase-1 duplication is resolved.
- `HSiKANBackbone` is now a thin `hg_state` adapter subclassing `signed_kan.SignedKANBackbone` (builds the signed
...
- Full suite: **301 passed, 2 skipped** (`hymeko_rl/tests/` + `signed_kan/tests/`, 4m12s).
...
## Next (phase 3)
Migrate `signedkan_wip` onto the core (sparse backend), **gated on the OTC regression** (AUC ≥ 0.8738 / macro-F1 ≥
