...
| Tuner source | `hymeko_neuro/run_gomb_tune.py` (`--joint-mix`, `for_joint_mix_tuning`) |
| Smoke runner | `hymeko_neuro/run_gomb_smoke.py` |
| Tests | `hymeko_neuro/tests/test_hymeko_gomb.py` |
...
```bash
cd /path/to/hymeko_framework_rust && PYTHONPATH=. python -m hymeko_neuro.run_gomb_tune \
