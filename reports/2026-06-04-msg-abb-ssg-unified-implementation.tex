...
% Companion to reports/2026-06-04-msg-abb-ssg-unified-implementation.md
...
\lstdefinestyle{ruststyle}{
    style=shellstyle,
    morekeywords={pub, fn, let, mut, use, struct, enum, impl, trait,
                  where, Box, Vec, Option, Self, dyn, ref, mod, crate,
                  super, self, match, if, else, return, true, false},
}
\lstdefinestyle{pythonstyle}{
    style=shellstyle, language=Python,
}
\lstset{style=shellstyle}

\title{%
  Unified reference: MSG / ABB / SSG implementations across the
  hymeko stack%
}
\author{%
...
\noindent\textbf{Scope.} A single-document map of the three
implementations of the Friedler 1992 MSG / ABB / SSG trio that the
\texttt{hymeko} codebase carries, their file-level pointers, and the
bridges between them. Companion to the 2026-05-18 dossier for Jean
Pimentel (\texttt{reports/2026-05-18-pimentel-abb-ssg-msg-dossier.md}):
the dossier explained \emph{what the algorithms are}; this document
explains \emph{where they live in the source tree, how they are wired
...
\section{Three application domains, one algorithmic trio}
...
\centering
\small
\begin{tabular}{p{0.16\linewidth}p{0.25\linewidth}p{0.20\linewidth}p{0.16\linewidth}p{0.16\linewidth}}
\toprule
Layer & Domain & Score / cost & ABB semantics & SSG semantics \\
\midrule
\textbf{P-graph synthesis} & PSE: subset of operating units producing required materials from raws & sum of per-unit fixed costs $c(u)$ & cost-minimal $O' \subseteq O_\mathrm{MSG}$ (or top-K) & enumerate every feasible $O'$ \\
\textbf{Signed-cycle} & HSiKAN cycle pool & scorer over $(vs, \mathrm{signs})$: balance, fraction-negative, entropy, sign-product-abs & top-K cycles by scorer, admissible-UB DFS prune & (deferred) \\
\textbf{Signed-walk} (2026-06-03) & HSiKAN walk pool & same scorer family & top-K walks by scorer, admissible-UB DFS prune & Pareto-subset over (primary, secondary) scorer pair \\
\bottomrule
\end{tabular}
\caption{The three application domains.}
\label{tab:domains}
\end{table}
...
prunes via an admissible bound against a running incumbent or top-K
heap; SSG returns either every feasible structure (P-graph) or the
Pareto-optimal subset (walks).

\section{Algorithmic invariants}

\subsection{MSG --- candidate generation with structural feasibility filter}

The MSG layer returns structures (operating-unit subsets, cycles,
walks) that pass the layer's feasibility test. For P-graphs the test
is the canonical Friedler axiom set (A1--A5;
\texttt{hymeko\_pgraph/src/axioms.rs}). For signed cycles and walks
the test is graph topology plus simple-path constraint (no vertex
revisit).

When the candidate space is too large to materialise (e.g.\ a
$4 \times 10^9$-walk DFS on bitcoin\_alpha walk\_len=4), MSG returns
a uniform random sample of size \texttt{cap} via reservoir sampling.
The walk-side Rust path uses Vitter Algorithm L; the Python
reference falls back to Algorithm R when the Rust extension is
unavailable.

\subsection{ABB --- pruned DFS with admissible bound}

Every ABB layer maintains an \emph{incumbent} --- the best feasible
solution found so far (P-graph) or the worst-of-top-K (cycles,
walks). At every DFS extension the layer computes an
\emph{admissible upper bound} on the score that any completion of
the current prefix could achieve. If the bound is dominated by the
incumbent, the entire subtree is pruned.

The admissibility postcondition is the load-bearing correctness
invariant:
\begin{equation}
  \forall \text{ closed structure completing prefix } q:\quad
  \mathrm{score}(\mathrm{completion}) \le
  \mathrm{upper\_bound}(\mathrm{prefix\ state}, \mathrm{remaining}).
\end{equation}
...
(\texttt{tests/abb\_global\_topk.rs::ub\_admissible\_*} for cycles;
\texttt{signedkan\_wip/tests/test\_abb\_msg\_ssg.py::test\_path\_scorer\_admissibility}
for walks). Four lemmas in
\texttt{paper/abb\_msg\_ssg\_signed\_walks/algorithms.tex \S4.4}
prove admissibility for the four walk-side
\texttt{PathScorer} implementations.

\subsection{SSG --- enumeration vs Pareto subset}

P-graph SSG is \emph{exhaustive}: it enumerates every feasible
$O' \subseteq O_\mathrm{MSG}$. The brute implementation
(\texttt{hymeko\_pgraph/src/ssg.rs}) iterates the
$2^{|O_\mathrm{MSG}|}$ subset lattice and refuses on
$|O_\mathrm{MSG}| > 30$. The canonical Friedler decision-mapping
(\texttt{hymeko\_pgraph/src/ssg\_dm.rs}) recurses over per-material
production decisions and is the only path that scales to the book's
Example 3.3 (29 units, 3{,}465 feasible structures).

Walk-side SSG is \emph{Pareto subset}: it filters the
ABB(primary scorer) top-K pool to the rows that are not dominated on
any of a secondary set of scorers. This is the multi-objective
extension. With a single objective the filter degenerates to the
ABB output.

\section{Implementation pointers}

\subsection{P-graph (Rust: \texttt{hymeko\_pgraph} crate)}
...
|-- axioms.rs            A1-A5 Friedler axioms (canonical 1992
|                        semantics restored 2026-05-19) + certificate
|-- regime.rs            Regime trait: Canonical | NoExcess |
|                        CostDominance | Composite (+-joined)
|-- msg.rs               maximal_structure[_with_regime]
|-- ssg.rs               enumerate[_with_options] -- brute 2^|O_MSG|
|                        subset lattice (refuses on |O_MSG| > 30)
|-- ssg_dm.rs            enumerate -- canonical Friedler 1992
|                        decision-mapping (Ch. 5, Def. 5.1)
|-- abb.rs               solve, solve_with_options,
|                        solve_with_regime, solve_top_k,
|                        solve_top_k_with_regime (2026-06-03)
...
|                        to_dot, to_friedler_dot (canonical PSE
|                        rendering, 2026-06-03), render_graphviz
`-- bin/
    |-- hymeko_pgraph_dump.rs   CLI: --algorithm msg|ssg|abb
    |                          + --top-k N --ssg-algorithm dm
    `-- pgraph.rs               CLI: read | transform | solve |
                                generate (+ --style friedler --format pdf)
\end{lstlisting}

Tests: \texttt{hymeko\_pgraph/tests/} --- 141 tests across 12 files
(including the 2026-06-03
\texttt{pimentel\_distractors.rs} four-test suite).

\subsection{Signed cycles (Rust: \texttt{hymeko\_graph::topk\_cycles})}
...
`-- cycle_enum.rs                enumerate_simple_cycles + serial
                                 reference
\end{lstlisting}

Python: \texttt{signedkan\_wip/src/cycle\_cache/api.py::cached\_construct\_k}
$\to$ routes to \texttt{hymeko.enumerate\_top\_k\_cycles\_rs} per
\texttt{HSIKAN\_TOPK\_MODE} env var.

\subsection{Signed walks (NEW, 2026-06-03)}

\begin{lstlisting}
hymeko_graph/src/topk_walks.rs       NEW
    enumerate_top_k_walks                serial DFS + admissible-UB
                                         prune + min-heap top-K
    enumerate_top_k_walks_batch          SoA output for zero-copy
                                         PyO3
...
|                                        (Algo L, numpy buffer, zero
|                                         per-offer alloc)
|-- path_scorers.py                NEW   PathScorer ABC + 4 concrete:
|                                        FractionNegative, Balance,
|                                        SignProductAbs, ShannonEntropy
|                                        + admissibility contract
|-- abb_walks.py                   NEW   abb_enumerate_walks (Python
|                                        ref + Rust delegation via
|                                        _try_rust_top_k_walks) +
|                                        msg_enumerate_walks +
|                                        ssg_pareto_filter
`-- walks.py                       upd   construct_walks_arrays uses
...
signedkan_wip/src/cycle_cache/strategies.py  NEW
    EnumeratedArrays                Output dataclass
                                    (v, sigma, edge_signs, is_closed)
    TupleEnumerator (ABC)           enumerate + cache_key_params
    CycleEnumerator
    WalkEnumerator = MSGWalkEnumerator   Vitter-L reservoir via Rust if
    ...
    ABBWalkEnumerator               composes PathScorer; delegates to
                                    ...
                                    when available
    SSGWalkEnumerator               ABB(primary) then Pareto-filter
                                    on (primary, secondary) scorer pair
    CachedEnumerator                Decorator: disk-cache + lazy pool
                                    wrapper
    cached_construct(g, kind, ...)  Single-line dispatcher
\end{lstlisting}

\section{Cross-domain bridges}

\subsection{The \texttt{BoundedScorer} trait bridges cycles and walks}

\texttt{hymeko\_graph::topk\_cycles::BoundedScorer} is defined once
and used in both the cycle and the walk path:
...
The contract is identical: \texttt{(vs, signs)} is the closed
structure's vertex / edge-sign sequence; \texttt{(n\_neg\_so\_far,
k\_remaining, k\_len)} is the prefix state.

\subsection{The Python \texttt{PathScorer} ABC mirrors \texttt{BoundedScorer}}
...
The four concrete implementations (\textsf{FractionNegativeScorer},
\textsf{BalanceScorer}, \textsf{SignProductAbsScorer},
\textsf{ShannonEntropyScorer}) have the same closed-form score and
upper-bound as the Rust path. \texttt{name()} is the bridge into the
Rust dispatcher (\texttt{enumerate\_top\_k\_walks\_rs(\dots,
score\_kind=name)}); only the four names listed get routed to Rust,
anything else falls back to the Python DFS reference.

\subsection{The Friedler \texttt{CyclePruner} pattern will transfer to walks (planned)}

The cycle-side \texttt{hymeko\_graph::pruner::CyclePruner} trait
(\texttt{extend\_ok} during DFS + \texttt{emit\_ok} at closure) is
the structural analogue of Friedler's two-stage check. The walk-side
currently hard-codes the only structural check (no vertex revisit)
inside the DFS; a planned refactor lifts this to a
\texttt{WalkPruner} trait symmetric to \texttt{CyclePruner}. See
\texttt{docs/plans/2026-06-03-abb-msg-ssg-walk-enumeration/plan.md}.

\section{Tests and cargo conformance}

\begin{table}[h]
\centering
\small
\begin{tabular}{p{0.30\linewidth}p{0.36\linewidth}rl}
\toprule
Suite & File & N & Purpose \\
\midrule
P-graph book conformance & \texttt{hymeko\_pgraph/tests/book\_validation.rs} & 5 & Friedler / Orosz / Pimentel-Losada book Examples 3.2/3.3/4.1/6.1/14.1 \\
P-graph decision-mapping SSG & \texttt{hymeko\_pgraph/tests/ssg\_decision\_mapping.rs} & 5 & Including Ex. 3.3 = 3{,}465 structures \\
Pimentel distractor fixture & \texttt{hymeko\_pgraph/tests/pimentel\_distractors.rs} & 4 & MSG=7, SSG=19, ABB top-3 = 9/12/13 \\
Walk ABB Rust & \texttt{hymeko\_graph/src/topk\_walks.rs::tests} & 8 & Brute-force equivalence over walk\_len $\times$ top\_k matrix \\
Walk MSG / ABB / SSG Python & \texttt{signedkan\_wip/tests/test\_abb\_msg\_ssg.py} & 16 & Admissibility, ABB-vs-brute, SSG Pareto \\
Reservoir samplers & \texttt{signedkan\_wip/tests/test\_reservoir.py} & 20 & Algorithm R + L correctness \\
Strategy adapters & \texttt{signedkan\_wip/tests/test\_tuple\_enumerator.py} & 18 & Strategy + Adapter round-trips, legacy wrapper equivalence \\
Cycle cache & \texttt{signedkan\_wip/tests/test\_cycle\_cache.py} & 13 & LazyCyclePool, pack/unpack symmetry \\
malloc\_trim helper & \texttt{signedkan\_wip/tests/test\_malloc\_trim.py} & 5 & Glibc heap release \\
Pre-existing Rust & \texttt{hymeko\_graph/src/*.rs::tests} & $\sim$75 & Cycle enumerators, balance pruners, Friedler axiom pruner \\
\bottomrule
\end{tabular}
\caption{Test inventory across the three layers.}
\end{table}

\textbf{Total reproducible:} 141 \texttt{hymeko\_pgraph} tests + 113
\texttt{hymeko\_graph} tests + 72 \texttt{signedkan\_wip} tests =
\textbf{326 tests} across the three layers, all green at the
2026-06-03 freeze.

\section{CLI surface (reproducible without Python imports)}

\subsection{P-graph analysis}
...
\subsection{Book conformance suite}
...
\subsection{HSiKAN cycle / walk pool (Python)}

\begin{lstlisting}[style=pythonstyle]
from signedkan_wip.src.cycle_cache import cached_construct

# MSG (uniform reservoir sample)
pool = cached_construct(g, "msg_walk",
                          walk_len=4, max_walks=100_000)
...
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item \textbf{Pimentel benchmark}
        (\texttt{reports/2026-06-03-pimentel-benchmark-reply.\{md,tex,pdf\}}):
        10-unit distractor problem reproduces MSG = 7, SSG = 19,
        ABB top-3 = (9, 12, 13); canonical PSE renderings in App.~C.
  \item \textbf{Friedler / Orosz / Pimentel-Losada book}
        (\texttt{scripts/pgraph/run\_examples.py}):
        8/8 canonical examples (Chapter 3.2, 3.3, 4.1, 4.3, 5.1, 6.1,
        14.1, HDA, methanol) match.
  \item \textbf{Signed-cycle ABB}
        (\texttt{reports/2026-05-10-abb-global-topk.md}):
        $25\times$ wall reduction on Epinions $k=4$ $K=10^4$.
  \item \textbf{Signed-walk ABB} (this paper, \S6 forthcoming):
        Slashdot 5-seed edge\_cr Komondor audit reproduces published
        SOTA $0.9067 \pm .0029$ (chain 13885723: $\{0.9098, 0.9049,
        0.9008, 0.9091, 0.9043\}$ mean $0.9058 \pm .0033$ within
        $+1\sigma$); BA + OTC + Epinions in flight.
\end{itemize}

\section{Open work}

\begin{enumerate}[leftmargin=2em,itemsep=0pt]
  \item \textbf{WalkPruner trait} symmetric to \texttt{CyclePruner} ---
        currently the simple-walk no-revisit check is hard-coded in
        \texttt{hymeko\_graph::topk\_walks::dfs}; lifting it to a
        trait would parallel the cycle-side
        \texttt{friedler::FriedlerAxiomPruner} and make walk-side
        regime composition possible.
  \item \textbf{Rayon-parallel walks} --- the current Rust DFS is
        serial; the cycle-side
        \texttt{enumerate\_top\_k\_per\_vertex\_cycles\_par\_bb\_batched}
        pattern (per-vertex DFS + thread-local heap + atomic global
        incumbent) is the template.
  \item \textbf{Python binding for the P-graph trio} --- the CLI
        binary is the only integration surface for non-Rust callers.
        A PyO3 wrapper exposing \texttt{MSG / SSG / ABB / regime} is
        on the planned-work list.
  \item \textbf{SSG-walk skyline} --- \texttt{pareto\_filter} is
        brute $O(N^2 D)$; sort-and-sweep (Kung–Luccio–Preparata
        1975) lowers to $O(N \log^{D-1} N)$ for large pools.
\end{enumerate}

\appendix

\section{Algorithm code listings (the actual implementation)}
\label{app:code}
...
\subsection{P-graph MSG --- bipartite fixpoint reduction}
\label{app:code-pgraph-msg}

Source: \texttt{hymeko\_pgraph/src/msg.rs}. The function performs a
two-phase fixpoint reduction over the bipartite IR: a forward
feasibility pass that drops units producing a raw material or
consuming an unavailable input, then a backward composition pass
keeping only units that reach a required product. A regime
refinement step on the output handles \texttt{NoExcess} /
\texttt{CostDominance}.
...
/// Forward closure: smallest set C \supseteq R such that whenever
/// a unit in `units` has all its inputs in C, its outputs are
/// added. Used by both the ABB reachability bound and SSG's
/// per-material decision recursion.
...
\subsection{P-graph SSG --- canonical decision-mapping recursion}
\label{app:code-pgraph-ssg}

Source: \texttt{hymeko\_pgraph/src/ssg\_dm.rs}. The Friedler 1992
algorithm (Ch.\ 5, Def.\ 5.1, Fig.\ 5.13) generates each
solution-structure exactly once by recursing over per-material
production decisions. The producer query
$\Delta(x) = \{u : x \in \mathrm{outputs}(u)\}$ is queried over the
\texttt{LoweredPGraph::producers} accessor.
...
impl Enumerator<'_> {
    ...
    fn recurse(
        ...
        // Candidate decisions: every non-empty subset of \Delta(x).
        for mask in 1u32..(1u32 << k) {
            ...
            //   include-consistency: chosen \cap excluded = \emptyset
            //   exclude-consistency: dropped \cap included = \emptyset
...
\subsection{P-graph ABB --- DFS with inclusion + reachability bounds}
\label{app:code-pgraph-abb}

Source: \texttt{hymeko\_pgraph/src/abb.rs}. The recursion is a
two-bound branch-and-bound: \textbf{inclusion bound} (partial cost
$\geq$ incumbent) and \textbf{reachability bound} (optimistic
remaining set fails to produce a required product). At every depth
the algorithm branches into include / exclude on the next unit.
...
fn branch(
    ...
    //
    // The optimistic remaining-units set is included \cup undecided.
...
\subsection{Signed-walk ABB --- DFS with min-heap top-K and admissible-UB prune}
\label{app:code-walk-abb}

Source: \texttt{hymeko\_graph/src/topk\_walks.rs}. Mirrors the cycle
ABB but for open walks: no closure check, canonical-form filter
\texttt{walk[0] <= walk[walk\_len]}, min-heap of size $K$ on
\textsf{BoundedScorer}.\textit{score}.
...
\subsection{NumpyReservoirSampler --- Vitter Algorithm L on a numpy buffer}
\label{app:code-reservoir}

Source: \texttt{signedkan\_wip/src/core/reservoir.py}. The walk-side
MSG primitive: zero per-offer Python allocation, expected RNG cost
$O(K \log(N/K))$ via Algorithm L. The hot path on a rejected offer
is a single integer compare and an increment.
...
\subsection{PathScorer ABC + the four concrete scorers}
\label{app:code-scorers}

Source: \texttt{signedkan\_wip/src/core/path\_scorers.py}. Each
scorer must satisfy the admissibility postcondition; the four
admissibility proofs appear in \S5 of the paper.
...
\subsection{Python ABBWalkEnumerator strategy + Rust delegation}
\label{app:code-abb-walk-strategy}

Sources: \texttt{signedkan\_wip/src/cycle\_cache/strategies.py}
(strategy class) + \texttt{signedkan\_wip/src/core/abb\_walks.py}
(\texttt{abb\_enumerate\_walks} with the Rust delegation shim).
...
\subsection{SSG-walk Pareto filter (multi-axis)}
\label{app:code-ssg-walk}

Source: \texttt{signedkan\_wip/src/core/abb\_walks.py}. The brute
$O(N^2 D)$ skyline filter on the ABB(primary) pool.
...
\section{Document genealogy}

This document subsumes the implementation-pointer content of
the 2026-05-18 Pimentel dossier, the early P-graph brief
(\texttt{reports/pgraph\_hymeko\_brief.tex}), the walk-side
framework plans (\texttt{docs/plans/2026-06-03-*/plan.md}), the
Pimentel benchmark validation trail
(\texttt{reports/2026-06-03-pimentel-benchmark-validation.md}), the
external Pimentel reply
(\texttt{reports/2026-06-03-pimentel-benchmark-reply.\{md,tex,pdf\}}),
and the paper drafts
(\texttt{paper/abb\_msg\_ssg\_signed\_walks/*.tex}).

For \emph{what the algorithms are} refer to the Pimentel dossier and
the paper. For \emph{where they live in the source tree} and
\emph{how to invoke them} this document is canonical.
