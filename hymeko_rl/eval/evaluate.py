...
# --- Pluggable-metric rollout: the ONE loop the eval_* family (delivery / lift-place / reach / balance) shares.
# The only thing that differed was the per-step metric; that is now a strategy (CLAUDE.md §6.1 / §6.5 #1).

class RolloutMetric(Protocol):
    """Per-step accumulator over a greedy rollout. ``reset`` at episode start; ``on_step`` each step (return
    ``True`` to STOP the episode early, e.g. a physics blow-up); ``finalize`` → this episode's value."""
...
def greedy_action_fn(ac: Any) -> ActionFn:
    """Adapt a policy's deterministic ``action_mean`` to the ``(env, obs) -> action`` ``ActionFn`` convention."""
    import torch

    def fn(_env: Any, obs: np.ndarray) -> np.ndarray:
        with torch.no_grad():
            out: np.ndarray = ac.action_mean(torch.as_tensor(obs[None], dtype=torch.float32)).squeeze(0).numpy()
        return out
    return fn


def eval_metric(env: Any, action_fn: ActionFn, metric: RolloutMetric, *, n_episodes: int,
                seed0: int = 0) -> "list[Any]":
    """Roll ``action_fn`` for ``n_episodes`` (distinct seeds), accumulating ``metric`` per episode. Returns the
    per-episode finalized values; the caller aggregates (rate / mean / mean-std). The single shared eval loop."""
    out: list[Any] = []
    for ep in range(n_episodes):
        obs, _ = env.reset(seed=seed0 + ep)
        metric.reset()
        for _ in range(env.max_steps):
            obs, reward, term, trunc, info = env.step(action_fn(env, obs))
            stop = metric.on_step(env, info, float(reward), bool(term or trunc))
            if stop or term or trunc:
                break
        out.append(metric.finalize())
    return out


class FlagSeenMetric:
    """1 if ``info[key]`` was ever truthy this episode (e.g. coin ``in_zone``), else 0 — an at-least-once rate."""

    def __init__(self, key: str) -> None:
        self.key, self.seen = key, False

    def reset(self) -> None:
        self.seen = False

    def on_step(self, _env: Any, info: "dict[str, Any]", _r: float, _done: bool) -> bool:
        self.seen = self.seen or bool(info.get(self.key, False))
        return False

    def finalize(self) -> int:
        return int(self.seen)


class DwellMetric:
    """1 if ``info[key]`` was truthy for ``min_consecutive`` CONSECUTIVE steps — the target was **held**, not
    just grazed. Mirrors the env's own sustained-success rule (``_success >= success_steps``), so a coin that
    instantaneously hits the zone and rolls out does NOT count as a delivery. Prefer this over
    :class:`FlagSeenMetric` for any "stayed in the target" success (measured need, 2026-07-01)."""

    def __init__(self, key: str, min_consecutive: int) -> None:
        if min_consecutive < 1:
            raise ValueError("min_consecutive must be >= 1")
        self.key, self.need = key, int(min_consecutive)
        self.consec = 0
        self.held = False
    ...
    def on_step(self, _env: Any, info: "dict[str, Any]", _r: float, _done: bool) -> bool:
        self.consec = self.consec + 1 if bool(info.get(self.key, False)) else 0
        if self.consec >= self.need:
            self.held = True
        return False

    def finalize(self) -> int:
        return int(self.held)


class FinalScalarMetric:
    """The last ``info[key]`` scalar of the episode (e.g. final end-effector distance)."""

    def __init__(self, key: str) -> None:
        self.key, self.last = key, float("nan")

    def reset(self) -> None:
        self.last = float("nan")

    def on_step(self, _env: Any, info: "dict[str, Any]", _r: float, _done: bool) -> bool:
        self.last = float(info[self.key])
        return False
...
class StepCountMetric:
    """Number of steps survived this episode (e.g. upright frames for the inverted pendulum)."""
    ...
    def on_step(self, _env: Any, _info: "dict[str, Any]", _r: float, _done: bool) -> bool:
        self.n += 1
        return False
...
class ReturnMetric:
    """Summed episode reward (e.g. the quadruped goal-reach return) — for tasks with no single success flag."""
    ...
    def on_step(self, _env: Any, _info: "dict[str, Any]", reward: float, _done: bool) -> bool:
        self.ret += reward
        return False
...
class LiftPlaceMetric:
    """``(lifted?, placed?)`` for the pick task; STOPS the episode on a physics blow-up so a divergence cannot
    fake a lift (the 2026-06-30 guard, now reusable). Reads ``env.data.qacc`` for the divergence check."""

    def __init__(self, *, lift_thresh: float = 0.035, diverge_qacc: float = 5_000.0) -> None:
        self.lift_thresh, self.diverge_qacc = lift_thresh, diverge_qacc
        self.lifted = self.placed = False
    ...
    def on_step(self, env: Any, info: "dict[str, Any]", _r: float, _done: bool) -> bool:
        ...
        if not np.isfinite(qacc) or qacc > self.diverge_qacc:
            return True                                            # diverged — pre-blow-up flags stand
        self.lifted = self.lifted or bool(info["lifted"] > self.lift_thresh)
        self.placed = self.placed or bool(info["reached"])
        return False
