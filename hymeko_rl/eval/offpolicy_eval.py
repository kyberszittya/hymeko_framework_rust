...
(``evaluate.evaluate``). No new env, no new trainer, no copied train loop (CLAUDE.md §6.5).
    ...
    python -m hymeko_rl.eval.offpolicy_eval --task galambos --algo sac --mode smoke   # 1 seed, cheap
    python -m hymeko_rl.eval.offpolicy_eval --task galambos --algo sac --mode full    # 5 seeds, the result
"""
...
from hymeko_rl.eval.evaluate import evaluate, now_stamp
from hymeko_rl.experiments.hsikan_diagnose import Diagnosis, diagnose
from hymeko_rl.agents.policy import POLICY_KINDS
from hymeko_rl.train.ppo import PPOEvalConfig, build_ppo, train_ppo_eval
from hymeko_rl.experiments.reach_arch_compare import _iqr   # one IQR helper for the whole package (no re-impl)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.plot and journal is not None and journal.exists():
        from hymeko_rl.viz.campaign_viz import plot_metric
