...
:mod:`hymeko_rl.viz.render_reach`: an :data:`ActionFn` (expert / bc / random / zero) steps the
...
time. Where ``render_reach`` is an offscreen sink (GIF/MP4), this is the live sink.
    ...
    python -m hymeko_rl.viz.viewer --robot data/robotics/anthropomorphic_arm.hymeko --source expert
    python -m hymeko_rl.viz.viewer --source random --no-realtime       # quick "is it alive" check
...
from hymeko_rl.viz.render_reach import (
