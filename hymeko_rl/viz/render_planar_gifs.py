...
seed, so runs can be compared side by side. Reuses :func:`hymeko_rl.eval.evaluate.render_episode_gif`
(the env-agnostic offscreen renderer) + a top-down camera matching the planar table.

    python -m hymeko_rl.viz.render_planar_gifs --checkpoint checkpoints/galambos/ppo_freed.pt \
...
from hymeko_rl.eval.evaluate import render_episode_gif, run_episode
...
def demonstrator_action_fn(demo: object):  # type: ignore[no-untyped-def]
    """Action source for the scripted :class:`~hymeko_rl.experiments.galambos_demo.GalambosDemonstrator` (reset it per
