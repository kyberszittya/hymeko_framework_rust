...
def test_strategy_spec_parses_galambos_strategy() -> None:
    ...
    # exploration tactic
    assert s.ent_coef == 0.01 and s.log_std_init == -0.5 and s.curriculum_iters == 60
    # exploitation
    assert (s.gamma, s.lam, s.clip, s.lr) == (0.99, 0.95, 0.2, 0.0003)
    assert (s.update_epochs, s.n_steps, s.n_iters) == (8, 512, 150)


def test_strategy_spec_builds_ppo_config() -> None:
    s = StrategySpec.from_hymeko(_STRATEGY)
    cfg = s.ppo_config(seed=7)
    assert cfg.n_iters == 150 and cfg.n_steps == 512 and cfg.ent_coef == 0.01
