...
The activation source is :meth:`hymeko_rl.policy.HSiKANBackbone.node_activations`. The current ``hg_state`` is
...
def mean_star_entropy(backbone: StructuralBackbone, obs: torch.Tensor, *, normalize: bool = True,
    ...
    # Preconditions ``backbone`` exposes ``node_activations(obs) -> (B, N, H)`` (i.e. an
      :class:`~hymeko_rl.policy.HSiKANBackbone`); ``obs`` is ``(B, N, in_feat)``.
