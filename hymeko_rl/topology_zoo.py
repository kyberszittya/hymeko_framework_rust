...
:class:`~hymeko_rl.hypergraph_state.HypergraphState` (random ±1 per undirected edge, both directions) so the
...
shortcuts), random ``G(n,p)``, complete. Reuses :func:`hymeko_rl.structural_probe.build_chain_graph` for the
chain (no duplicate).
"""
...
def _hyper_lift(builder: "Callable[..., HypergraphState]", tag: str) -> "Callable[..., HypergraphState]":
    ...
    from hymeko_rl.hypergraph_designs import graph_to_kuniform
