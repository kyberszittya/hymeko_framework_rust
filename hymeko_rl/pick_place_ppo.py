...
generic :func:`hymeko_rl.ppo.train_ppo` (the same trainer the reach + cart-pole scenarios use).

    python -m hymeko_rl.pick_place_ppo --kind hsikan --iters 80
"""
...
from hymeko_rl.gripper_pick_bc import build, collect_demos, eval_success
...
def run_pick_place_ppo(kind: str = "hsikan", *, hidden: int = 64, seed: int = 0, n_eval: int = 8,
    ...
    history = train_ppo(ac, env, cfg, on_iteration=on_iter)
    lift, place = eval_success(fanuc_pick_env(), ac, n_eval, seed=20_000)
    return dict(policy=kind, n_params=ac.n_parameters(), warm_start=pretrain_demos > 0,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(run_pick_place_ppo(a.kind, seed=a.seed, cfg=cfg, pretrain_demos=a.pretrain_demos), indent=2))
    return 0
