...
and zero-pads the per-vertex obs for the design hubs. No copied trainer, no per-arm code (§6.5).
    ...
    python -m hymeko_rl.exp_designed_control --plant pendulum --mode smoke   # harness validation (rung 0/1)
    python -m hymeko_rl.exp_designed_control --plant quadruped --mode full   # the thesis test (rung 2)
"""
...
from hymeko_rl.evaluate import experiment_dir, plot_scoreboard
from hymeko_rl.exp_pernode_actor_ab import run_config
from hymeko_rl.hypergraph_designs import steiner_blocks, sunflower_cover
from hymeko_rl.reach_arch_compare import _iqr
from hymeko_rl.sac import SACConfig
...
def _register_plants() -> None:
    ...
    try:
        # NOTE: QuadrupedGoalEnv is a *locomotion* (walk-to-goal) env. Rung 2 wants *standing* (balance), so a
        # stay-upright reward is the task to build before this is the thesis test — registered here so the
        # body set / harness is reachable in the meantime.
