...
to :func:`hymeko_rl.agents.policy._catmull_rom` (a parity test pins them together).
...
def fused_catmull_rom(x: torch.Tensor, coef: torch.Tensor) -> torch.Tensor:
    ...
    # Postconditions returns ``(..., C)``, bit-close to :func:`hymeko_rl.agents.policy._catmull_rom`.
    """
