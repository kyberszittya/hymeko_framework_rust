"""Canonical planar 2-link arm kinematics — production, experiment-free.

Analytic inverse kinematics + per-arm extraction for the planar two-arm table used across the coin/galambos scripted
controllers. Extracted verbatim (behaviour-preserving) from ``experiments.galambos_demo`` so the production controllers
(:mod:`hymeko_rl.agents.cooperative_contact_controller`, …) can build the position-servo command without importing the
galambos/coin-toss experiment web. Depends only on ``math`` / ``mujoco`` / ``numpy``.

Public API:
    * :func:`planar_2link_ik` — analytic IK for one planar 2-link arm.
    * :class:`ArmKin` — per-arm kinematics + action wiring resolved from a stepped model.
    * :func:`extract_arms` — resolve both arms (base→link1→link2→tip) and their action indices.
    * :func:`ik_action` — map per-arm fingertip targets to the 4-DoF position-servo command via per-arm IK.
"""
...
def extract_arms(model: Any) -> dict[str, ArmKin]:
    ...
    for base in range(model.nbody):
        ...
        arms[side] = ArmKin(side, base_xy, l1, l2, tip, act_of_joint[int(j_sh[0])], act_of_joint[int(j_el[0])])
    if "left" not in arms or "right" not in arms:
        raise ValueError(f"could not resolve both arms; found {sorted(arms)}")
    return arms


def ik_action(pairs: "Iterable[tuple[ArmKin, np.ndarray]]", env: Any) -> np.ndarray:
