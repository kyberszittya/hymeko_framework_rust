"""Train the HSiKAN actor-critic (or the MLP baseline) to balance the HyMeKo cart-pole.
    ...
    python -m hymeko_rl.train_inverted_pendulum --policy hsikan
    python -m hymeko_rl.train_inverted_pendulum --policy mlp      # the ablation baseline
...
from hymeko_rl.policy import POLICY_KINDS, ActorCritic, build_policy
...
def _make_balance_policy(kind: str, env: InvertedPendulumEnv, hidden: int) -> ActorCritic:
    ...
    :func:`hymeko_rl.bc._make_policy`, sized from the gym spaces so it is env-contract-agnostic.
...
def eval_balance(env: InvertedPendulumEnv, ac: GreedyPolicy, n_episodes: int, *, seed: int) -> float:
    """Mean upright-steps over ``n_episodes`` greedy (deterministic-mean) rollouts.
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            obs, _, terminated, truncated, _ = env.step(mean.squeeze(0).numpy())
            n += 1
