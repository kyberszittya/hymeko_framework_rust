...
def run(n_step: int, seed: int, rep: int, steps: int, out: Path, critic_mode: str = "TASK_ONLY",
        actor_head: str = "pooled") -> dict:
    ...
    actor, critics = build_sac("mlp", obs_dim=41, flat_dim=41, action_dim=6, action_scale=1.0, actor_head=actor_head)
    _src = torch.load(_CKPT, map_location="cpu")
    if isinstance(actor, ContactActorBank):      # F21: warm-start BOTH heads from the F11 curriculum policy (clone)
        warm_start_contact_bank(actor, _src)
    else:                                        # F11: load the single-actor curriculum checkpoint directly
        actor.load_state_dict(_src)
    ...
    _cell = ("F21" if actor_head == "contact_bank" else "F12" if critic_mode == "TASK_AND_MECHANISM" else "F11")
    arm = f"{_cell} n{n_step} s{seed}r{rep}"
    ...
    def eval_fn(_e, ac):
        # F21: consume the mode occupancy + per-head action magnitude accumulated over the last training window (§5)
        bank_diag = ac.pop_diagnostics() if isinstance(ac, ContactActorBank) else None
        ...
        m = dict(eval=len(hist) + 1, step=(len(hist) + 1) * 2500, stage1=s1, stage2=s2,
                 s64102_strict=r64102, strong_strict=strong_ok, bc_coef=bc_coef_fn(0), bank_diag=bank_diag)
        ...
        occ = (f" | occT={bank_diag['mode_occupancy_transport']:.2f} magT={bank_diag['mag_transport']:.2f} "
               f"magR={bank_diag['mag_reposition']:.2f}") if bank_diag else ""
        print(f"  [{arm} ev#{len(hist)}] S2:cov{s2['coverage']}/{s2['n']} L{s2['loose']} mc{s2['max_certified_clearance']:+.2f} "
              f"prog{s2['mean_progress']:.3f} | S1ret:cov{s1['coverage']} strong={strong_ok} 64102={r64102}{occ}", flush=True)
    ...
    result = dict(n_step=n_step, seed=seed, rep=rep, steps=steps, critic_mode=critic_mode, actor_head=actor_head,
                  cell=_cell, source_checkpoint=str(_CKPT), strong_state_hash=_STRONG_HASH, curve=curve,
...
def main() -> None:
    ...
    ap.add_argument("--actor-head", choices=["pooled", "contact_bank"], default="pooled",
                    help="pooled = F11 single actor; contact_bank = F21 two contact-mode actors (HYMeko_CONTACT_MODE)")
    ap.add_argument("--seed", type=int, required=True)
    ap.add_argument("--rep", type=int, required=True)
    ap.add_argument("--steps", type=int, default=50_000)
    ap.add_argument("--out", required=True)
    a = ap.parse_args()
    run(a.nstep, a.seed, a.rep, a.steps, Path(a.out), critic_mode=a.critic_mode, actor_head=a.actor_head)
