...
from hymeko_rl.coin_delivery.delivery_teacher.solver import solve_delivery_with_settle
...
def coverage_gate(rows: list, bank_path: Path = R11_4A_BANK) -> dict:
    ...
    passed = _coverage_passed(overall, cov, rec_split, nudge_only, safety_ok, energy_ok)
    return {"scenarios": len(rows), "certified": len(done), "teacher_recovered": len(rec),
            ...
            "verdict": ("R11_5_TARGET_CONDITIONED_DELIVERY_TEACHER_COVERAGE_PASS" if passed
                        else "R11_5_TARGET_CONDITIONED_DELIVERY_TEACHER_COVERAGE_INSUFFICIENT")}
...
def main() -> None:
    ...
    with args.out.open("w", encoding="utf-8") as fh:
        for i, (sid, split) in enumerate(fails, 1):
            ...
            r = run_one(rig, cfg, conf, obj, lambda snap, s: solve_delivery_with_settle(snap, s), sid, split)
