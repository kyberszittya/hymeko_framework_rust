...
needs a real CTDE trainer with per-agent rollouts — BC supervises the joint action and so cannot exercise it.
    ...
    python -m hymeko_rl.experiments.exp_collaborative --seed 0
    python -m hymeko_rl.experiments.exp_collaborative --smoke
"""
...
from hymeko_rl.agents.collaborative import build_collaborative
...
def _refine(policy: Any, env: PlanarGraspEnv, refine: int, seed: int, *, bc_coef: float = 0.0,
            bc_data: "tuple[Any, Any] | None" = None,
            make_env: "Callable[[], PlanarGraspEnv] | None" = None, n_envs: int = 8) -> None:
    ...
    refine near the demos (the BC anchor cross-ported from pick-place). Vectorized rollout is the measured-faster
    default (no curriculum here, so it applies); ``n_envs=1`` keeps the single-env path."""
    if refine > 0:
        train_ppo(policy, env, PPOConfig(n_iters=refine, n_steps=2048, seed=seed, ent_coef=0.003,
                                         bc_coef=bc_coef), bc_data=bc_data,
                  n_envs=n_envs, make_env=make_env if n_envs > 1 else None)
...
def run(*, seed: int, n_demos: int, bc_epochs: int, hidden: int, difficulty: float, n_eval: int,
    ...
    _mk = lambda: _env(difficulty, task_graph)   # noqa: E731 — vectorized-rollout worker factory
    _refine(hs, env, refine, seed, bc_coef=bc_coef, bc_data=(obs, acts), make_env=_mk)
    ...
    _refine(coll, env, refine, seed, bc_coef=bc_coef, bc_data=(obs, acts), make_env=_mk)
