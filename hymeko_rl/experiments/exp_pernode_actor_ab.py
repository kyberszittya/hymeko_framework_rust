...
(:class:`hymeko_rl.train.sac.PerNodeSquashedGaussianActor`) instead of mean-pool → ``Linear(feat, action_dim)``.
...
loop, no re-implemented rendering (§6.1).
    ...
    python -m hymeko_rl.experiments.exp_pernode_actor_ab --task galambos --mode smoke   # proves plumbing + 3-form output
    python -m hymeko_rl.experiments.exp_pernode_actor_ab --task galambos --mode full    # the result (1 seed)
"""
...
from hymeko_rl.eval.offpolicy_eval import _env_dims, match_mlp_hidden
