...
seconds-long toy bandit (:mod:`hymeko_rl.sanity_rl`) and returns a rank + a recommendation, so only survivors get
...
**Two things it deliberately does NOT screen** (stated so it cannot mislead):
  ...
  * **holonomy advantage** — the toys are acyclic/linear; use :mod:`hymeko_rl.holonomy_probe` for that.
...
from hymeko_rl.sanity_rl import BanditConfig, run_bandit_sanity
