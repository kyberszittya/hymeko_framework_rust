...
use hymeko_framework::ir::ir::{DeclKind, Ir, SignedRefR};
use hymeko_framework::resolution::interner::Interner;
use hymeko_framework::resolution::module_store::{CompiledProgram, HymekoParser, ModuleStore};
use hymeko_framework::resolution::resolve::Index;
...
fn fmt_hash_opt(h: Option<HashId>) -> String {
    match h {
        Some(h) => to_hex(&h.0),
        None => "None".to_string(),
    }
